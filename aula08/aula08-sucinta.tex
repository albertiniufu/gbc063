...
%%  mesmas definições e figuras do aula08.tex)
...
\begin{frame}{Onde estamos}{Da recompensa dada à recompensa aprendida}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    ...
    \item \textbf{Aula 7:} sem recompensa escrita? Primeira resposta:
      \emph{copie um especialista}.
    \item \textbf{Hoje:} por que copiar não basta; \emph{inferir} a recompensa;
      \emph{preferências humanas} --- o caminho até o RLHF e o DPO.
  \end{itemize}

  \begin{ideiabox}
    A partir de hoje $\rec$ vira \emph{objeto de aprendizado} --- quase todos
    os problemas novos nascem daí.
  \end{ideiabox}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{Recapitulação da aula passada}
  ...
  \begin{respbox}
    ...
    política determinística nunca amostra as demais ações → sem gradiente →
    presa onde começou.
  \end{respbox}
\end{frame}
  ...
  \begin{column}{0.49\textwidth}
    \begin{enumerate}\setlength{\itemsep}{0.5ex}
      \item \textbf{Imitação e erros compostos}\\
        {\small por que clonar degrada como $T^2$; o que o DAGGER faz}
      \item \textbf{Aprendizado de recompensa}\\
        {\small RL inverso, não unicidade de $\rec$, máxima entropia}
      \item \textbf{Humanos no laço}\\
        {\small espectro de esforço humano; comparações baratas}
    \end{enumerate}
  \end{column}
  \begin{column}{0.49\textwidth}
    \begin{enumerate}\setlength{\itemsep}{0.5ex}
      ...
      \item \textbf{DPO}\\
        {\small eliminar o modelo de recompensa por álgebra}
    \end{enumerate}
  \end{column}
  \end{columns}

  \begin{ideiabox}
    Fio condutor: ``de onde vem o sinal de recompensa quando não há placar?''
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Por que isto virou o assunto central}{RL como etapa final de treinamento de modelos de linguagem}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Duas primeiras etapas: supervisionadas. Terceira: RL --- e é a que
      exige que exista uma \emph{recompensa}.
    \item Ninguém sabe escrever ``responda de forma útil e honesta'' como uma
      $\rec(s,a)$; todos sabem dizer \emph{qual de duas respostas é melhor}.
  \end{itemize}
\end{frame}
...
\begin{frame}{O problema de aprendizagem por imitação}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item \textbf{Clonagem de comportamento} (Aula 7): classificação ---
      aprenda $\pol(a\mid s)$ por perda supervisionada sobre os pares
      $(s,a)$.
    \item Simples, rápido, sem interação com o ambiente --- resultados
      históricos (ALVINN, 1989) e atuais (o ``SFT'' dos LLMs).
  \end{itemize}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{O que a clonagem de comportamento supõe --- e o que ela ignora}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \end{defbox}
    \smallskip
    No MDP, falso duas vezes: estados de uma trajetória \textbf{dependentes}
    no tempo; a distribuição deles depende da \textbf{política} que os gera.
  \end{column}
  ...
  \begin{ideiabox}
    O problema estrutural da imitação: as ações do aluno mudam as perguntas da
    prova. {\scriptsize\color{rlCinza}\;(Ross \& Bagnell, 2011.)}
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Erros independentes no tempo: o caso benigno}
  ...
  Suponha que, a cada passo, a política clonada erre com probabilidade
  ...
  \begin{ideiabox}
    Agente ``volta ao trilho'' sozinho após cada erro: custo \textbf{linear}
    no horizonte --- a intuição do supervisionado.
  \end{ideiabox}

  Pergunta da aula: \emph{o que muda quando um erro desloca a trajetória?}
\end{frame}
...
\begin{frame}{Erros compostos: o custo real}
  ...
  \begin{column}{0.53\textwidth}
    ...
    Sem garantia --- pessimismo: \emph{todos} os $T-t$ passos restantes
    comprometidos:
    \begin{align*}
      \E[\text{erros}] &\lesssim
  ...
  \begin{column}{0.45\textwidth}
    ...
    \end{center}
    \begin{alertabox}
      Com $\varepsilon = 1\%$ e $T = 1000$: $10$ vs.\ $\sim\!10^{4}$ erros. A
      política ``com 99\% de acerto'' é inutilizável.
    \end{alertabox}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{A geometria do problema}
  ...
  \begin{ideiabox}
    O erro não é ``ruído em torno da resposta certa'': é \textbf{desvio que se
    realimenta} --- longe do suporte dos dados, pior a política; pior a
    política, mais longe ela vai.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \begin{respbox}
    Reduz $\varepsilon$, mas \textbf{não muda o expoente}. Mais dados do
    motorista são mais dados da região \emph{onde ele dirige bem} --- e o
    problema está fora dela. Ataca a causa obter rótulos \textbf{nos estados
    que a política aprendida visita}.
  \end{respbox}
\end{frame}
...
\begin{frame}{DAGGER: agregação de conjuntos de dados}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Ideia: \emph{rótulos do especialista ao longo do caminho que a
      política aprendida percorre}.
    \item Política determinística com bom desempenho \textbf{sob a
      distribuição de estados que ela mesma induz}.
    \item Redução a aprendizado \emph{online} sem arrependimento --- o custo
      volta a ser $O(\varepsilon T)$.
  \end{itemize}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{DAGGER: o algoritmo}
  ...
  \begin{ideiabox}
    A mistura $\beta_i$ evita tempo demais em estados catastróficos no início,
    quando $\pol_i$ ainda é ruim --- o instrutor que segura o volante.
  \end{ideiabox}
\end{frame}
  ...
  \begin{respbox}
    ...
    \begin{itemize}\setlength{\itemsep}{0.1ex}
      \item \textbf{Custo}: a supervisão escala com estados visitados, não com
        demonstrações.
      \item \textbf{Contexto}: ``o que você faria \emph{agora}?'' num quadro
        congelado produz rótulos ruins.
...
\begin{frame}{Balanço da imitação}
  ...
  \begin{ideiabox}
    Última linha: a \emph{recompensa} é a descrição mais \textbf{compacta e
    transferível} da tarefa. Mude o carro ou a estrada: a política se refaz;
    a intenção, não.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Inverter o problema}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    \item Recompensa: descrição \textbf{mais sucinta} da tarefa (Ng \&
      Russell, 2000); recuperada, replaneja-se em outra dinâmica.
    \item Antes de qualquer algoritmo: uma pergunta de identificabilidade.
  \end{itemize}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{Teste sua compreensão}
  ...
  \begin{respbox}
    ...
    problema é mal posto --- e o RL inverso consiste em escolher \emph{qual}
    solução preferir.
  \end{respbox}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{As três fontes de ambiguidade}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \end{alertabox}
    \begin{alertabox}
      \textbf{2. Escala.} Se $\rec$ explica $\polE$, $c\,\rec$ com
  ...
  \begin{column}{0.48\textwidth}
    \begin{alertabox}
      ...
      \]
      preserva \emph{exatamente} as políticas ótimas
  ...
  \begin{ideiabox}
    Invariância 2 reaparece em Bradley--Terry: recompensa de LLM sem escala
    absoluta. Escolher entre infinitas soluções: \textbf{estrutura} +
    \textbf{critério de desempate}.
  \end{ideiabox}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{Estrutura: recompensa linear em atributos}
...
\begin{frame}{Desempate: o princípio da máxima entropia}
  ...
  \begin{ideiabox}
    Leitura: especialista \textbf{aproximadamente} ótimo --- trajetórias
    melhores exponencialmente mais prováveis, mas erros possíveis.
  \end{ideiabox}
...
\begin{frame}{MaxEnt IRL: como se aprende $w$}
  ...
  \begin{alertabox}
    Laço interno: \emph{resolver um MDP inteiro a cada passo de gradiente}.
    Guided Cost Learning (Finn et al., 2016) troca por amostragem por
    importância --- equivalência com GANs: a recompensa é o discriminador.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Balanço: aprendizagem por imitação}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item \textbf{É poderosa.} Demonstrações: a forma mais natural de
      comunicar uma tarefa a uma máquina, e existem em abundância.
    \item \textbf{Muitas extensões}: imitação a partir de observação (sem
      ações), com um único exemplo, a partir de vídeo humano.
    \item \textbf{Máxima entropia é a ideia central}: transformar
      ``explicar comportamento'' em verossimilhança bem posta.
    \item \textbf{Mas} pressupõe um especialista que \emph{sabe fazer} a
      tarefa. E se as pessoas só souberem \emph{reconhecer} uma execução boa?
  \end{itemize}

  \begin{ideiabox}
    Ponte: escrever um soneto é difícil; dizer qual de dois é melhor, nem
    tanto --- o RLHF é construído sobre essa assimetria.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Como uma pessoa pode ajudar a treinar um agente}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    \item Formas: demonstrar, corrigir, pontuar, comparar, restringir,
      interromper.
    \item A escolha importa: agentes que igualem o desempenho humano
      \textbf{e} respeitem valores humanos.
  \end{itemize}
\end{frame}
...
\begin{frame}{Recompensa dada por pessoas: o caminho ingênuo}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item \textbf{TAMER} (Knox \& Stone, 2008): ``bom'' ou ``ruim'' enquanto o
      agente age; treina-se um modelo do reforço humano.
    \item \textbf{Robôs ensináveis} (Thomaz \& Breazeal, 2008): pessoas
      \emph{não} dão reforço como a teoria supõe --- antecipam, premiam
      intenção e \emph{orientam}, não só avaliam.
  \end{itemize}

  \begin{alertabox}
    Pedir números falha por \textbf{calibração} (o ``7'' de uma pessoa não é o
    de outra, nem o dela na semana seguinte), \textbf{ruído} (variância alta) e
    \textbf{esforço} (atenção contínua).
  \end{alertabox}

  \begin{ideiabox}
    Saída (psicometria, pré-RL): não pergunte \emph{quanto}; pergunte
    \emph{qual dos dois}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Comparações par a par}
  ...
  \begin{column}{0.49\textwidth}
    \begin{itemize}\setlength{\itemsep}{0.3ex}
      \item Recomendação há décadas: \emph{interleaving} mais sensível que
        medir cada ordenação.
  ...
  \begin{alertabox}
    Perda: comparações dão apenas uma \textbf{ordem}, e RL precisa de um
    \emph{número}. Reconstruir a escala é o papel do modelo de Bradley--Terry.
  \end{alertabox}
\end{frame}
...
\begin{frame}{O modelo de Bradley--Terry (1952)}
  ...
  Caso simples: $k$ ações $b_1,\dots,b_k$, sem estado nem contexto (um
  \emph{bandido} de $k$ braços). Comparações \textbf{ruidosas}, recompensa
  latente $r(\cdot)$:
  \[
  ...
  \begin{column}{0.52\textwidth}
    \begin{itemize}\setlength{\itemsep}{0.2ex}
      \item \emph{Softmax} entre dois itens --- ou regressão logística sobre a
        \textbf{diferença} de recompensas.
      \item \textbf{Transitividade estocástica}: $p_{ik}$ fica determinado por
        $p_{ij}$ e $p_{jk}$ --- hipótese forte, testável.
    \end{itemize}
  \end{column}
...
\begin{frame}{A recompensa latente não é única --- de novo}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Sem hipóteses adicionais, o modelo latente de recompensa não é
      identificável (mesma lição do RL inverso).
    \item Bradley--Terry: a \textbf{hipótese estrutural} que reduz a
  ...
  \begin{alertabox}
    ...
    \emph{mesmo} prompt são interpretáveis --- por isso a normalização por
    prompt é padrão na implementação.
  \end{alertabox}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}[shrink]{Que item é ``o melhor''? Três respostas}
  ...
  \begin{ideiabox}
    Bandidos ``em duelo'' buscavam o vencedor de Copeland. \textbf{Sob
    Bradley--Terry os três critérios coincidem} (transitividade estocástica
    elimina ciclos) --- quando o modelo \emph{falha}, a escolha importa.
    {\scriptsize\color{rlCinza}(Yue et al., JCSS 2012.)}
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Ajustando os parâmetros: entropia cruzada}
  ...
  Suponha $N$ tuplas $(b_i, b_j, \mu)$: $\mu = 1$ se a pessoa marcou
  $b_i \succ b_j$, $0{,}5$ em caso de empate, $0$ caso contrário. Maximizar a
  verossimilhança = minimizar a entropia cruzada:
  \[
  ...
  \begin{ideiabox}
    Isto é \textbf{regressão logística} com atributos $e_i - e_j$ e
    coeficientes $r$ --- convexa, solução única dada a normalização.
  \end{ideiabox}
...
\begin{frame}{De itens para trajetórias}
  ...
  Mesma construção trocando ``item'' por \emph{trajetória}: $\traj^1 =
  (s_0, a_7, s_{14},\dots)$ e $\traj^2 = (s_0, a_6, s_{12},\dots)$, somas de
  recompensas latentes $R^1 = \sum_{i} r^1_i$ e $R^2 = \sum_i r^2_i$:
  \[
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Agora $r$ é uma \textbf{rede neural} $\rphi(s,a)$, treinada pela
      mesma perda de entropia cruzada.
    \item Com $\rphi$ em mãos: RL comum --- rode PPO com a recompensa
      aprendida.
  \end{itemize}

  \begin{alertabox}
    O que a soma implica: a preferência é atribuída à trajetória \emph{inteira},
    mas o modelo aprende uma recompensa \emph{por passo} --- \textbf{atribuição
    de crédito}, agora dentro do sinal de supervisão.
  \end{alertabox}
\end{frame}
...
\begin{frame}{O resultado que abriu a porta}
  ...
  \begin{column}{0.55\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{0.3ex}
      \item Tarefa: agente simulado executa um \emph{backflip} --- sem
        recompensa escrita possível.
      \item Interface: dois clipes curtos; qual está mais próximo do desejado.
      \item Custo total: cerca de \textbf{900 comparações} de um avaliador
        humano.
      \item Recompensa treinada \emph{em paralelo} com a política; consultas
        onde ele está mais incerto.
    \end{itemize}
  \end{column}
  \begin{column}{0.43\textwidth}
    \begin{ideiabox}
      Novecentos bits de feedback compraram o que nenhuma recompensa escrita à
      mão havia produzido: \emph{avaliar} é muito mais barato que
      \emph{especificar}.
    \end{ideiabox}
    \begin{alertabox}
      O mesmo algoritmo, sem mudança conceitual, cinco anos depois alinharia
...
\begin{frame}{O \emph{pipeline} de três etapas}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    \item Etapa 1: exatamente a clonagem de comportamento da Aula 7.
    \item Etapas 2 e 3: as duas metades de ``como maximizar algo que não
      sabemos escrever?''
  \end{itemize}
\end{frame}
...
\begin{frame}{Etapa 2: aprender o modelo de recompensa}
  ...
  Coleta: prompt $x$, várias respostas com $\psft$, pessoa ordena um par.
  $\yw$ preferida (\emph{winning}), $\yl$ preterida (\emph{losing}).
  ...
  \begin{column}{0.52\textwidth}
    \begin{itemize}\setlength{\itemsep}{0.2ex}
      \item $\rphi$: o próprio LLM com a camada final trocada por uma saída
        escalar.
      \item Uma resposta inteira recebe \textbf{um} número: a soma de
        recompensas por token do slide anterior colapsa numa recompensa
        terminal.
    \end{itemize}
  \end{column}
  \begin{column}{0.46\textwidth}
    \begin{ideiabox}
      ...
      Acurácia em julgamentos humanos retidos; modelos grandes com dados
      suficientes se aproximam da concordância entre dois humanos --- o teto
      real.
    \end{ideiabox}
  \end{column}
  \end{columns}
...
\begin{frame}{Etapa 3: otimizar sem destruir o modelo}
  ...
  \begin{alertabox}
    ...
    dados finitos, válido apenas onde $\psft$ gera texto. Maximizá-la sem
    restrição leva o modelo a \emph{explorar as falhas do avaliador} --- o
    fenômeno de \emph{reward hacking}. A KL mantém a política dentro do domínio
    onde $\rphi$ ainda é confiável.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Sobre-otimização: a lei de Goodhart, com números}
  ...
  \begin{column}{0.51\textwidth}
    ...
    \end{ideiabox}
    \begin{itemize}\setlength{\itemsep}{0.2ex}
      \item $\beta$: o controle desse compromisso.
...
\begin{frame}{O que se observou na prática}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item \textbf{RLHF supera pré-treino $+$ ajuste supervisionado} em avaliação
      humana --- vantagem cresce com a escala
      {\scriptsize(Stiennon et al., 2020)}.
    \item \textbf{InstructGPT} (Ouyang et al., 2022): o \emph{pipeline} em
      dezenas de milhares de tarefas; modelo muito menor, alinhado, foi
      preferido a um bem maior sem alinhamento.
    \item \textbf{Comparações controladas} (Dubois et al., 2023), GPT-4 como
      avaliador substituto: PPO funciona, mas linhas de base simples ---
      \emph{best-of-$n$} e treinar sobre as saídas ``boas'' --- vão
      surpreendentemente longe.
  \end{itemize}

  \begin{ideiabox}
    Lição de engenharia: boa parte do ganho vem de \textbf{ter um bom modelo de
    recompensa}, não da sofisticação do otimizador. RL é a parte cara e frágil:
    dá para eliminá-la?
  \end{ideiabox}
\end{frame}
...
%% ------------------------------------------------------------
\begin{frame}[shrink]{O que incomoda no RLHF}
  ...
  \begin{column}{0.49\textwidth}
    \begin{ideiabox}
      A recompensa é só um \emph{intermediário} entre preferências e política.
      Relação em forma fechada → \textbf{substituir uma pela outra} e treinar a
      política direto sobre os pares. {\scriptsize\color{rlCinza}(Rafailov et
      al., 2023.)}
    \end{ideiabox}
  \end{column}
  \end{columns}
...
\begin{frame}{Passo 2: inverter a relação}
  ...
  \begin{ideiabox}
    ...
    logarítmica entre uma política e a referência --- positiva se a política
    gosta da resposta \emph{mais} que o modelo de referência, negativa se gosta
    menos.
  \end{ideiabox}
...
\begin{frame}{Passo 3: substituir --- e ver $Z$ desaparecer}
  ...
  Perda \textbf{supervisionada} sobre um conjunto \emph{fixo} de pares --- sem
  amostragem, sem crítico, sem modelo de recompensa explícito.
\end{frame}
...
\begin{frame}{O que o gradiente do DPO faz}
  ...
  \begin{ideiabox}
    Gradiente de política ``suba o que deu certo, desça o que deu errado'' ---
    com \textbf{peso adaptativo}: quase zero nos pares que o modelo já ordena
    corretamente, esforço nos que ele ainda erra. É o que impede o colapso da
    máxima verossimilhança ingênua sobre $\yw$.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O que se ganha e o que se perde}
  ...
  \begin{alertabox}
    ...
    escreveu. Daí as variantes \emph{iterativas} (geração, rotulagem, DPO).
  \end{alertabox}
\end{frame}
...
\begin{frame}{Evidência empírica e adoção}
  ...
  \begin{column}{0.49\textwidth}
    \begin{itemize}\setlength{\itemsep}{0.3ex}
      \item Com recompensa de ouro conhecida, a comparação é limpa: sem dúvida
        sobre a qualidade do modelo de recompensa.
      \item Em escala, DPO e variantes: padrão em famílias abertas (Zephyr,
        Mistral, Llama 3).
      \item Extensões próximas: \textbf{CPL}, mesma ideia para preferências
        sobre \emph{trajetórias} de controle, fora da linguagem.
    \end{itemize}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O mapa depois do DPO}
  ...
  \begin{ideiabox}
    Ramo de cima move o estado da arte em raciocínio: correção \emph{verificável}
    (uma prova, um teste unitário, uma resposta numérica) → recompensa dada de
    novo --- a sobre-otimização desta aula desaparece.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O que você deve levar desta aula}
  ...
  \begin{ideiabox}
    Moral: sem placar, alguém precisa fornecê-lo --- \emph{quem} fornece,
    \emph{a que custo}, \emph{quanto confiar}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Para ir além}
  ...
  \begin{column}{0.49\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{0ex}
      \item Preferências humanas: interseção de \textbf{escolha social},
        \textbf{economia computacional} e IA --- literatura anterior ao
        aprendizado de máquina.
      \item Teoremas de impossibilidade (Arrow) afetam ``alinhar'' um modelo a
        um \emph{grupo} com preferências conflitantes.
    \end{itemize}
  \end{column}
  \end{columns}

  \begin{alertabox}
    Pergunta que a técnica não responde: \emph{de quem} são as preferências
    usadas para rotular os pares. Escolha de projeto, não detalhe de
    implementação.
  \end{alertabox}
\end{frame}
