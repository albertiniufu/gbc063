...
%%  Compilar com XeLaTeX (2x) — latexmk -xelatex aula05-sucinta.tex
%% ============================================================
...
\begin{frame}{Onde estamos}{Do valor à política}
  ...
  \begin{itemize}\setlength{\itemsep}{0.5ex}
    \item \textbf{Aulas 2--3:} modelo dado → $\Vstar$ e $\polstar$ por
      programação dinâmica.
    \item \textbf{Aula 4:} sem modelo, \emph{aprender} $\Qhat(s,a;\wv)$ de
      amostras --- MC, SARSA, Q-learning, DQN.
    \item \textbf{Hoje:} descartar o intermediário --- parametrizar
      \emph{diretamente} $\polth(a\mid s)$; subir o gradiente do retorno
      esperado.
    \item \textbf{Próxima aula:} passo de gradiente confiável --- gradiente
      natural, TRPO, PPO.
  \end{itemize}

  \begin{ideiabox}
    Política: de \emph{subproduto} do valor ($\pol(s) = \argmax_a \Qhat(s,a)$)
    a objeto de primeira classe; valor, quando aparecer, só auxiliar contra
    variância.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Recapitulação: o que ficou da aula passada}
  ...
  Aula 4, sem modelo: atualização incremental rumo a um \emph{alvo}:
  \[
  ...
  \begin{alertabox}
    A \textbf{tríade mortal} --- aproximação de função $+$
    \emph{bootstrapping} $+$ off-policy --- tira as garantias; o DQN não a
    resolve: contorna com repetição de experiência e alvos fixos.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Três famílias de algoritmos}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Hoje: coluna do meio; ator--crítico no fim da aula, quase de graça.
    \item Notação: $\wv$ = pesos da \emph{função valor}; $\thv$ = pesos da
      \emph{política}.
  \end{itemize}
\end{frame}
...
\begin{frame}{A política parametrizada}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Discreto: rede → \emph{logits} → \emph{softmax}.
    \item Contínuo: rede → média (e talvez desvio) de gaussiana.
    \item Continuamos \textbf{sem modelo}: $\tran{s'}{s,a}$ nunca será
      necessário --- cai sozinho da matemática.
  \end{itemize}
\end{frame}
...
\begin{frame}{Por que não bastam políticas determinísticas}{Três motivos}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.55ex}
    \item \textbf{Ações contínuas.} $\argmax_a \Qhat(s,a)$ = otimização
      \emph{dentro} de cada passo; $\polth$: ação sai numa passagem para
      frente.
    \item \textbf{Estados perceptualmente ambíguos.} Estados indistinguíveis →
      melhor política pode exigir \emph{aleatoriedade} --- exemplo a seguir.
    \item \textbf{Suavidade.} $\epsilon$-guloso muda descontinuamente quando
      dois $\Qhat$ se cruzam; $\polth$ muda suave com $\thv$ → convergência
      mais estável.
  \end{enumerate}

  \begin{ideiabox}
    Quarto motivo, moderno: um modelo de linguagem \emph{já é} uma distribuição
    sobre tokens --- RLHF, PPO e GRPO.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Exemplo: o corredor com estados sinônimos}
  ...
  Corredor de cinco casas; agente enxerga só atributos locais:
  \[
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Casas 2 e 4: paredes N/S → \textbf{exatamente o mesmo vetor de
      atributos} --- o agente não as distingue.
    \item Comparar: $Q_{\thv}(s,a) = f(\feat(s,a);\thv)$ vs.\
...
\begin{frame}{O corredor: por que o determinismo falha}
  ...
  2 e 4 indistinguíveis → \emph{qualquer} política determinística escolhe a
  mesma ação nas duas:

  \begin{columns}[T]
  \begin{column}{0.5\textwidth}
    \begin{alertabox}
      \textbf{Sempre para o Leste:} casa 2 chega; casa 4 fica presa.
      \textbf{Sempre para o Oeste:} simétrico. \emph{Metade dos inícios nunca
      alcança o objetivo}.
    \end{alertabox}
  \end{column}
  \begin{column}{0.5\textwidth}
    \begin{ideiabox}
      RL baseado em valor ≈ política quase determinística. $\epsilon$ salva no
      limite, mas escapar custa $\sim 1/\epsilon$: milhares de passos.
    \end{ideiabox}
  \end{column}
  \end{columns}
  ...
  \begin{teobox}{A política ótima aqui é estocástica}
    ...
    passeio aleatório local alcança o objetivo em poucos passos de qualquer
    início.
  \end{teobox}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \begin{respbox}
    \textbf{Não há contradição --- o objeto mudou.} Teorema vale para MDPs,
    política do \emph{estado}; aqui, política da \emph{observação}
    $\feat(s,\cdot)$ → POMDP. Sob restrição de representação --- ou
...
\begin{frame}{O que exatamente vamos maximizar}
  ...
  \vspace{-0.6em}
  \begin{ideiabox}
    Segunda forma: \emph{otimização estocástica pura} --- derivação curta, e
    válida bem além de MDPs.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Notação de trajetória}
  ...
  \begin{alertabox}
    Depende de $\thv$: \textbf{apenas} os fatores $\polth(a_t\mid s_t)$ ---
    nem $\mu$, nem a dinâmica, nem $\rec$.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Antes do gradiente: por que não otimização sem gradiente?}
  ...
  Maximizar $V(\thv)$: otimização de caixa-preta. Há métodos \emph{sem}
  gradiente:
  ...
  \begin{teobox}{Por que insistir no gradiente}
    Diferenças finitas: $\mathcal{O}(n)$ avaliações \emph{ruidosas} por passo
    --- proibitivo com $n$ na casa dos milhões. O gradiente analítico custa
    \emph{uma} passagem para trás.
  \end{teobox}

  \begin{ideiabox}
    Competitivos com $n$ pequeno ou simulador barato e massivamente paralelo
    (Salimans et al., 2017).
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Subida de gradiente na política}
  ...
  Dificuldade: $V(\thv) = \sum_{\traj}\Ptraj \Rtraj$ --- soma sobre um número
  astronômico de trajetórias; $\Ptraj$ desconhecida.
\end{frame}
...
\begin{frame}{Estimador de Monte Carlo}
  ...
  Com $m$ trajetórias $\traj^{(1)},\dots,\traj^{(m)}$ da política atual:
  \[
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item \textbf{Não enviesado:} $\E[\hat{g}] = \gth V(\thv)$ exatamente, para
      qualquer $m$.
    \item Ainda parece inútil: $\log P(\traj;\thv)$ contém a dinâmica
      desconhecida.
    \item \textbf{Próximo slide:} abrir o logaritmo faz a dinâmica desaparecer.
  \end{itemize}

  \begin{ideiabox}
    Já ganhamos: ``derivar uma soma sobre todas as trajetórias'' → ``rodar a
    política e tirar uma média''. Resto da aula: reduzir a \emph{variância}
    dessa média.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Decompondo a trajetória: a dinâmica cai fora}
  ...
  \begin{teobox}{Nenhum modelo de dinâmica é necessário}
    Logaritmo transforma produto em soma; a derivada aniquila o que não
...
\begin{frame}{Gradiente de política por função escore}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.25ex}
    \item Rodar a política; guardar $(s_t,a_t,r_t)$.
    \item Somar as recompensas de cada trajetória.
    \item Derivar $\log \polth$ --- diferenciação automática.
  \end{enumerate}

  \begin{ideiabox}
    Sem modelo, sem $\max$ sobre ações. Em troca: \emph{episódios completos}
    e on-policy.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{A função escore}
  ...
  \begin{teobox}{Propriedade fundamental: o escore tem média zero}
    ...
    Identidade a guardar: é ela que prova que uma \emph{linha de base} não
    introduz viés.
  \end{teobox}
\end{frame}
...
\begin{frame}{Política softmax e seu escore}
  ...
  \begin{ideiabox}
    \textbf{Atributo observado menos atributo esperado.} Ação ``mais típica''
    que a média → escore pequeno; atípica → escore grande. × retorno alto,
    empurra a política para o que foi surpreendentemente bom.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Política gaussiana e seu escore}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item $\sigma^2$ fixa ou parametrizada --- na prática,
      \textbf{controle de exploração} que a otimização vai encolhendo.
    \item Redes profundas substituem a forma linear; derivação igual.
  \end{itemize}

  \begin{alertabox}
    $1/\sigma^2$ explode com $\sigma \to 0$: política quase determinística →
    gradientes gigantescos e instáveis. Modo de falha comum em controle
    contínuo --- limite $\sigma$ por baixo.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Intuição do estimador por função escore}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Andar em $\hat{g}_i$ \textbf{aumenta a log-probabilidade da amostra},
      na proporção do quão boa ela foi.
    \item $f$ pode ser \textbf{descontínua, desconhecida ou discreta}: nunca
      derivamos $f$, só $\log p$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Por que isso é notável: escore \emph{versus} reparametrização}
  ...
  \begin{ideiabox}
    Em RL a ``função'' é o \emph{mundo}: placar de jogo, preferência humana ---
    não há gradiente para pegar. Escore: caro em variância, barato em
    suposições --- e é o que sobrevive.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Teorema do gradiente de política}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Generaliza: no lugar do retorno da trajetória, $Q^{\polth}(s,a)$.
    \item \textbf{O teorema evita} $\gth d^{\pol}$: a distribuição de estados
      depende de $\thv$, mas o gradiente \emph{não} a contém --- resultado
      difícil (S\&B, cap.\ 13.2).
    \item Resto da aula: escolher \emph{o que pôr no lugar de $Q^{\polth}(s,a)$}.
  \end{itemize}
\end{frame}
...
\begin{frame}{O problema: não enviesado, mas ruidoso demais}
  ...
  \begin{alertabox}
    Não enviesado --- e \textbf{de variância altíssima}. Cada ação recebe o
    retorno da trajetória \emph{inteira}: uma ação excelente no passo 3 paga
    um desastre no passo 300.
  \end{alertabox}

  Três correções, nesta ordem; cada uma é ideia independente:
  \begin{enumerate}\setlength{\itemsep}{0.35ex}
    \item \textbf{Estrutura temporal} --- ação não influencia o passado.
    \item \textbf{Linha de base} --- comparar com o esperado, não com zero.
    \item \textbf{Alternativas ao retorno MC} --- variância por um pouco de
      viés (ator--crítico).
  \end{enumerate}
\end{frame}
...
\begin{frame}{Correção 1: o resultado}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Removemos os $r_{t'}$ com $t' < t$: em esperança valiam
      \textbf{zero} (ação não influencia o passado); só contribuíam com
      variância pura.
    \item Continua não enviesado, com variância estritamente menor.
  \end{itemize}

  \begin{ideiabox}
    Princípio geral: \emph{remova do estimador tudo que tem esperança zero}.
    Não muda o alvo e reduz ruído --- a linha de base, a seguir, é a mesma
    ideia.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{REINFORCE: gradiente de política por Monte Carlo}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Razão de verossimilhança $+$ estrutura temporal.
    \item \textbf{On-policy}; exige o episódio completo ($G_t$).
    \item 1992 --- ancestral direto de tudo que vem depois.
  \end{itemize}
\end{frame}
...
\begin{frame}{Por que ainda não funciona bem}{Um exemplo de três linhas}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Retornos grandes e positivos $\Rightarrow$ estimador \textbf{aumenta
      a log-probabilidade das duas ações} em toda amostra.
    \item Diferença útil --- o único sinal --- é $1$, sepultada num sinal de
      magnitude $100$.
    \item Some $+1000$ a todas as recompensas: o gradiente \emph{verdadeiro}
      não muda (escore tem média zero); a \textbf{variância explode}.
  \end{itemize}

  \begin{ideiabox}
    A escala absoluta não importa em esperança --- só no \emph{ruído}.
    Consertar isso é exatamente o papel da linha de base.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Correção 2: introduzir uma linha de base}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Quase ótima: $b(s_t) \approx \E[r_t + \dots + r_{T-1}] = \Vpi(s_t)$.
    \item \textbf{Leitura:} aumentar a log-prob de $a_t$ na proporção do
      retorno \emph{melhor que o esperado} naquele estado.
  \end{itemize}

  \begin{ideiabox}
    De ``esta ação rendeu 101'' para ``$0{,}5$ acima da média deste estado''.
    Segundo sinal é comparável entre estados; o primeiro, não.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{A linha de base não introduz viés --- demonstração}
  ...
  \begin{teobox}{Consequência}
    Subtrair $b(s_t)$ altera a variância, \textbf{não} a esperança: liberdade
    total na escolha de $b$. O segundo passo exige $b$ dependendo só de $s_t$.
  \end{teobox}
\end{frame}
...
\begin{frame}{O algoritmo ``baunilha'' de gradiente de política}
  ...
  \vspace{-0.6em}
  \begin{ideiabox}
    Dois problemas supervisionados alternando: regressão ($b$) e subida
    ponderada ($\polth$).
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Que linha de base escolher?}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    ...
    \item \textbf{Ótima em variância:} minimiza $\Var(\hat g)$; média dos
      retornos \emph{ponderada pelo escore ao quadrado},
      \[
  ...
  \begin{alertabox}
    Prática: também \textbf{normalização de vantagens} por lote
    ($\Ahat \leftarrow (\Ahat - \bar{\Ahat})/(\sigma_{\Ahat} + \varepsilon)$):
    viés pequeno; $\alpha$ deixa de depender da escala da recompensa.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \begin{respbox}
    \textbf{Não.} A demonstração de não-viés exige $b$ dependendo apenas de
    $s_t$; com $b$ dependendo de $a_t$ não se pode retirá-la da esperança
    sobre ações. Pior: o termo subtraído seria exatamente
    $\E[\gth\log\pol \cdot Q^{\pol}]$, o gradiente inteiro --- em esperança o
    estimador daria \textbf{zero}; a política nunca aprenderia.
  \end{respbox}
\end{frame}
...
\begin{frame}{Onde estamos e o que falta}
  ...
  \begin{alertabox}
    ...
    enviesada, mas com o ruído de todas as transições e escolhas de ação até
    o fim do episódio.
  \end{alertabox}

  \begin{ideiabox}
    Remédio da Aula 4 (MC vs.\ TD): \textbf{viés} via \emph{bootstrapping}
    compra uma queda grande de \textbf{variância}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Métodos ator--crítico}
  ...
  \begin{defbox}{Ator e crítico}
    ...
    avalia. Ambos são aprendidos e atualizados.
  \end{defbox}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    \item Crítico troca o retorno bruto por estimativa \emph{bootstrapped}:
      menos ruído, leve viés. A3C (Mnih et al., 2016) popularizou a
      arquitetura; A2C, PPO e SAC herdam.
  \end{itemize}
\end{frame}
...
\begin{frame}{A função vantagem}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    ...
    \item Positivo $\Rightarrow$ torne a ação mais provável; negativo
      $\Rightarrow$ menos. Sem centramento, ``todas as ações ficam mais
      prováveis''.
  \end{itemize}
\end{frame}
...
\begin{frame}{Estimadores de $n$ passos: um dial entre TD e MC}
  ...
  \begin{ideiabox}
    ...
    como sinal de aprendizado da \emph{política}. O mesmo objeto, dois papéis.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O compromisso viés--variância}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item $n$ pequeno: alvo depende de $\Vhat$, errado no início $\Rightarrow$
      \textbf{viés}.
    \item $n$ grande: ruído de muitas transições acumulado $\Rightarrow$
      \textbf{variância}.
    \item \textbf{GAE} (Schulman et al., 2016): média exponencial de todos os
      $n$ com peso $\lambda$:
      $\Ahat^{\text{GAE}}_t = \sum_{k\ge 0}(\gamma\lambda)^k \delta_{t+k}$.
      Padrão em PPO --- e é exatamente o $\lambda$ de TD($\lambda$).
  \end{itemize}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \begin{respbox}
    $\Ahat^{(1)}$: \textbf{variância baixa e viés alto} --- uma única
    transição, mas herda todo o erro de $\Vhat(s_{t+1})$.

    \smallskip
    $\Ahat^{(\infty)}$: \textbf{variância alta e viés baixo} --- recompensas
    reais até o fim (só $\Vhat(s_t)$, a linha de base, é aproximada --- e ela
    não introduz viés); custa o ruído de todo o episódio.
  \end{respbox}
\end{frame}
...
\begin{frame}{O algoritmo baunilha, versão final}
  ...
  \vspace{-0.4em}
  \begin{ideiabox}
    Família (REINFORCE, A2C, TRPO, PPO, GRPO): esqueleto + 2 escolhas ---
    \emph{estimar a vantagem}, \emph{limitar o passo}; próxima aula: a segunda.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Vantagens e desvantagens do RL baseado em política}
  ...
  \begin{column}{0.5\textwidth}
    \begin{alertabox}
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        ...
        \item \textbf{On-policy}: cada lote serve a uma atualização e é
          descartado.
      \end{itemize}
    \end{alertabox}
  \end{column}
  ...
  \begin{ideiabox}
    Prática: ações discretas e simulador caro $\Rightarrow$ DQN. Contínuas ou
    política necessariamente estocástica $\Rightarrow$ gradiente de política.
    Os dois $\Rightarrow$ ator--crítico off-policy (SAC, DDPG).
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Armadilhas práticas}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    \item \textbf{Passo $\alpha$ criticamente sensível.} Grande demais destrói
      a política; os dados seguintes vêm \emph{dela} --- não há como se
      recuperar. Motiva TRPO/PPO.
    \item \textbf{Colapso de entropia.} Política fica determinística cedo;
      exploração morre; gradiente vai a zero. Remédio usual: bônus de entropia
      $+\beta\,\mathcal{H}[\polth(\cdot\mid s)]$ no objetivo.
    \item \textbf{Normalize as vantagens} por lote; normalize também as
      observações.
    \item \textbf{Lotes grandes.} Muito mais amostras por atualização que
      supervisionado --- variância cai com $1/\sqrt{m}$.
    \item \textbf{Recompensas esparsas} continuam o pior caso: sem recompensa
      no lote, $\hat g \approx 0$ e nada acontece.
  \end{itemize}
\end{frame}
...
\begin{frame}{O que você deve levar desta aula}
  ...
  \vspace{-0.9em}
  \begin{ideiabox}
    Moral: ``aumente a probabilidade do que deu certo''; o resto: \emph{o que
    conta como ter dado certo}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Próxima aula}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    \item Estimador pronto; o \emph{passo} não. Quão longe andar em $\thv$ sem
      destruir a política que gera nossos dados?
    \item \textbf{Gradiente natural}: distância entre políticas no espaço de
      \emph{distribuições} (divergência KL), não no espaço de parâmetros.
    \item \textbf{TRPO} e \textbf{PPO}: melhoria monotônica aproximada
      limitando a KL --- e a versão barata, com razão de probabilidades
      recortada, que treina modelos de linguagem hoje.
  \end{itemize}
