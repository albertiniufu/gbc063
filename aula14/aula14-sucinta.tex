...
%%  mesmas definições e figuras do aula14.tex)
...
\begin{frame}{Plano de hoje}
  \begin{enumerate}\setlength{\itemsep}{0.35em}
    \item \textbf{Planejar agora, em vez de resolver tudo} — computação local por \emph{uma} jogada; MCTS usa o orçamento.
    \item \textbf{AlphaZero} — rollout vira rede de \emph{política} e \emph{valor}: PUCT, quatro fases, política da raiz.
    \item \textbf{Auto-jogo} — busca vira dados de treino; adversário = cópia de si.
    \item \textbf{A evidência} — arquitetura vs.\ busca vs.\ dados humanos; números do artigo.
    \item \textbf{Além do tabuleiro} — AlphaTensor, AlphaDev, MuZero: descoberta como busca.
    \item \textbf{Um olhar crítico} — UCT é o algoritmo certo para gastar simulações?
  \end{enumerate}
\end{frame}
...
\begin{frame}{Onde isto se encaixa no curso}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \small
      \begin{itemize}
        \item Antes: política \alert{para todo} o espaço de estados — iteração de valor, $Q$-learning, DQN, gradientes, PPO; $\pol : \gS \to \gA$.
        \item Hoje: dado $s_t$, computação para \alert{uma} ação boa. Depois, repetir.
        \item As visões não competem: AlphaZero usa uma para melhorar a outra.
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        Global = \emph{amortizada}; local = \emph{sob demanda}. $\abs{\gS}$ astronômico: só a local viável — a global vira aproximação corrigida na hora.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Busca Monte Carlo simples}{Um passo de melhoria de política, e nada mais}
  \small
  Modelo (simulador) $\mathcal{M}$ + política de simulação $\pol$:
  \begin{itemize}
  ...
  \begin{alertabox}
    \textbf{Um passo} de melhoria sobre $\pol$: guloso em $Q^{\pol}$, não $\polstar$. $\pol$ ruim $\Rightarrow$ estimativa ruim.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Podemos fazer melhor: a árvore expectimax}
  \footnotesize
  Com o modelo, dá para calcular $Q^{*}(s_t,a)$ \emph{exatamente} — expandir a árvore completa a partir de $s_t$ e retropropagar
  \[
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertabox}
        Árvore cresce como $(\abs{\gS}\abs{\gA})^{H}$.
...
\begin{frame}{Busca em árvore Monte Carlo: crescer a árvore onde importa}
  \small
  Resposta ao $(\abs{\gS}\abs{\gA})^{H}$: não construir a árvore toda — árvore \alert{assimétrica}, fundo só onde as simulações indicaram valer.
  ...
  \medskip
  A árvore da direita é o \emph{produto} do algoritmo, não sua entrada: cada simulação a torna informativa onde a decisão é difícil.
\end{frame}
...
\begin{frame}{As quatro fases de uma simulação}
  ...
  \medskip
  \small
  Repita $K$ vezes; ação real sai das estatísticas da raiz. \alert{Todo o desenho está em duas escolhas:} seleção (fase 1) e origem do $v$ (fase 3).
\end{frame}

%% ------------------------------------------------------------
\begin{frame}{UCT: cada nó da árvore é um bandit}
  \small
  Seleção: UCT (Kocsis e Szepesvári, 2006) — cada nó, ações como braços, cota superior de confiança.
  ...
  \begin{itemize}
    \item $\Nvis(i,a)$: simulações por $a$ em $i$; $G_k(i,a)$: $k$-ésimo retorno dali.
    \item \alert{A política de simulação muda a cada episódio} — a árvore se adapta ao já visto.
    \item $\overline{G}$: média sobre política \emph{não estacionária} — última seção.
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.4em}
        \item \textbf{Busca seletiva pelo melhor primeiro.} Orçamento onde a decisão aperta, não onde a árvore é larga.
        \item \textbf{Avaliação dinâmica de estados.} Valor do nó construído pelas simulações que passam.
        \item \textbf{Amostragem quebra a maldição da dimensionalidade.} Custo = orçamento, não $\abs{\gS}$.
        \item \textbf{Basta um modelo ``caixa-preta''.} Nada de $\tran{s'}{s,a}$ — só \emph{amostrar} transições.
        \item \textbf{Barato, \emph{anytime} e paralelizável.} Pare quando o relógio acabar; a resposta já serve.
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        ``Anytime'': iteração de valor nada entrega antes de convergir; MCTS entrega após a 1ª simulação. Xadrez, Go, tempo real: usável vs.\ inútil.
      \end{ideiabox}
      \begin{alertabox}
        Custo no orçamento: milhares de simulações por decisão — memória e pré-computação viram \emph{tempo de decisão}.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}
        ...
        \item Alfa-beta com avaliação manual \alert{fracassou} no Go, ao contrário do xadrez.
      \end{itemize}
      \medskip
      \begin{center}
      \tiny
    ...
    \begin{column}{0.46\textwidth}
      ...
      \vspace{-0.3em}
      \begin{respbox}
        \textbf{Não.} Regras conhecidas e determinísticas; recompensa verificável.
        Falta é \emph{usar} o modelo: $\abs{\gS}\approx 10^{170}$, $H \approx 150$.
        AlphaZero = \alert{planejamento} com heurísticas aprendidas.
      \end{respbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{De AlphaGo a AlphaZero}{Três sistemas, três graus de conhecimento humano embutido}
  ...
  \medskip
  \begin{ideiabox}
    Sequência de \emph{remoções} de muletas humanas; tirar a rollout — o ``Monte Carlo'' — \emph{melhorou}. A Bayesiana já deslocava o peso para a rede de valor (Chen et al., 2018).
  \end{ideiabox}
\end{frame}
...
\begin{frame}{A rede: uma função, duas saídas}
  \small
  Uma rede residual $f_{\theta}$ recebe $s$ (tabuleiro cru + histórico recente + vez) e devolve
  \[
  ...
  \]
  \begin{itemize}
    \item $\redepol(s)$ — \textbf{cabeça de política}: distribuição sobre as jogadas legais; \emph{prior} por onde olhar.
    \item $\redeval(s)$ — \textbf{cabeça de valor}: resultado final ($+1$ vitória, $-1$ derrota); substitui a rollout.
  \end{itemize}
...
\begin{frame}{PUCT: a regra de seleção do AlphaZero}
  \small
  Por ação, no nó: $\Nvis(s,a)$, $\Wtot(s,a)$, $Q(s,a)=\Wtot/\Nvis$ e o prior $P(s,a) = \redepol(s)_a$. Ação simulada:
  ...
  \begin{itemize}
    \item $\Nvis$ pequeno $\Rightarrow$ $U$ domina: busca segue o \alert{prior da rede}.
    \item Visitas crescem $\Rightarrow$ $U \to 0$, $Q$ assume: a busca \alert{corrige} a rede por lookahead.
    \item $\cpuct$ regula a transferência de poder.
  \end{itemize}
\end{frame}
...
\begin{frame}{UCT e PUCT lado a lado}
  ...
  \begin{alertabox}
    PUCT \textbf{não} é cota de confiança: prior baixo pode nunca ser visitado. Go: $\approx 250$ jogadas, 1\,600 simulações — viável, e exige o ruído de Dirichlet.
  \end{alertabox}
\end{frame}
...
\begin{frame}{A simulação, do começo ao fim}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{1. Seleção.} Da raiz, desça $\argmax_a [Q+U]$ até $s_L$ fora da árvore.

      \medskip
      \textbf{2. Expansão e avaliação.} Uma chamada $f_{\theta}(s_L) = (\redepol, \redeval)$; nó com $\Nvis=\Wtot=Q=0$, priors $\redepol$; $v = \redeval(s_L)$. \alert{Não há rollout.}
      ...
      \[
        ...
        Q = \Wtot/\Nvis .
      \]
      Sinal alterna com o jogador da vez.
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{ideiabox}
        \textbf{Rollout → rede = MC → TD.} Rollout: não enviesado vs.\ política de rollout, variância enorme, viés vs.\ $\polstar$. Rede: bootstrap, viés baixo que encolhe com o treino.
      \end{ideiabox}
...
\begin{frame}{O PUCT em câmera lenta}{Um exemplo mínimo que cabe no quadro}
  \small
  Raiz com três jogadas; a rede erra: prior alto em $a_1$, a pior das razoáveis. Folhas: $\redeval = (0{,}10;\ 0{,}55;\ -0{,}40)$; $\cpuct = 1{,}5$.
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35em}
        \item \textbf{$k=1$--$2$: a rede manda.} $Q\equiv 0$; escolha = prior.
        \item \textbf{$k=3$: a evidência entra.} $a_1$ devolve $0{,}10$, abaixo do prior → busca migra.
        \item \textbf{$k=7$: a busca volta a $a_1$.} $1+\Nvis(s,a_2)$ cresceu; $U$ confere.
        \item \textbf{$a_3$ fica de fora até $k=23$.} Prior baixo + orçamento pequeno = jogada nunca examinada.
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{alertabox}
        \textbf{Prior muito errado custa caro.} $P = (0{,}95;\ 0{,}04;\ 0{,}01)$, mesmo $\redeval$:
        \smallskip
        ...
        \end{center}
        \smallskip
        A busca \emph{corrige} a rede; preço = orçamento. Rede melhor $\Rightarrow$ menos simulações.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{$\cpuct$: quanto duvidar da própria rede}
  ...
  \medskip
  \begin{ideiabox}
    Não é cosmético: Bayesiana nos hiperparâmetros da MCTS rendeu \textbf{94 e 103 Elo} antes do Lee Sedol; 100 Elo $\approx$ 64\% de vitórias.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Ruído de Dirichlet: garantir que tudo possa ser tentado}
  \footnotesize
  Prior abre a porta; se a de $a_3$ nunca abre, a rede errada não é descoberta. Em \alert{auto-jogo} (não em partida oficial), perturba-se o prior \emph{apenas na raiz}:
  \[
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        Só na raiz: dá para tentar a estranha, descartável quando $Q$ mostra que é ruim — exploração que a evidência veta, oposto do $\epsilon$-guloso.
      \end{ideiabox}
      \begin{alertabox}
        Removido em avaliação e partidas oficiais: mecanismo de \emph{coleta de dados}, não de jogo.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{A política da raiz: contagens de visita, não valores}
  \footnotesize
  Terminadas as $K$ simulações, a política exportada na raiz:
  \[
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      No exemplo anterior, $\Nvis = (5,11,0)$:
      \medskip
      \begin{center}\small
      \begin{tabular}{cccc}
        \toprule
      ...
      \end{center}
      \medskip
      Auto-jogo: $\tau = 1$ nas 30 primeiras jogadas (diversidade), depois $\tau \to 0$.
    \end{column}
    \begin{column}{0.51\textwidth}
      ...
      \pause
      \begin{respbox}
        $\Nvis$ mais robusto que $Q$: visitada 3 vezes, ação pode ter $Q$ alto por sorte. Contagens incorporam a confiança.
      \end{respbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Por que a busca melhora a política}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \textbf{De onde vem.} $\redepol(s_0)$ julga por padrões; $\raizpol$ após olhar $K$ continuações — informação que $\redepol$ não tinha.
      \smallskip
      \textbf{Consequência.} Treinar $\redepol$ para imitar $\raizpol$ = passo de melhoria; repetir = iteração de política, busca no lugar do guloso.
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{ideiabox}
        \textbf{Deliberação treina intuição:} busca = pensar devagar; rede = pensar rápido — \emph{Expert Iteration} (Anthony, Tian e Barber, 2017).
      \end{ideiabox}
      \begin{alertabox}
        ``Tipicamente'' $\neq$ ``sempre'': $\redeval$ sistematicamente enviesado numa região → mais busca amplifica o erro.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O que o PUCT está realmente otimizando}{Grill et al., ICML 2020}
  \footnotesize
  Contagens não são subproduto arbitrário: resolvem aproximadamente um \alert{problema de otimização de política regularizada}:
  \[
  ...
  \vspace{-0.35em}
  \begin{ideiabox}
    $\lambda_{\Nvis} \sim \Nvis^{-1/2} \to 0$: pouca busca $\Rightarrow$ perto do prior; muita $\Rightarrow$ $\argmax$ de $q$. A ``proximidade regularizada'' do TRPO/PPO, agora \emph{dentro} de um planejador.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Os dois alvos, e por que só um é escasso}
  \small
  Cada jogada gera $(s_t,\ \raizpol_t,\ z)$, com $z \in \{-1,0,+1\}$ o resultado \emph{final} visto de $s_t$.
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{itemize}
        \item \textbf{Alvo de política: denso.} 200 jogadas → 200 alvos $\raizpol_t$ ricos (distribuições, não rótulos duros); melhoria destilada.
        \item \textbf{Alvo de valor: escasso.} Um bit por partida, replicado em todas as posições; exemplos correlacionados, sinal fraco.
      \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{alertabox}
        ``Go tem recompensa esparsa'' = meia verdade: alvo de treino da política é denso — a busca fabrica supervisão em cada posição. Gradiente puro patinaria.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Auto-jogo é um currículo automático}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.4em}
        \item \textbf{O gargalo é só computação.} Sem anotador, especialista ou banco de partidas.
        \item \textbf{Adversário sempre à altura} — cópia de si; vitórias $\approx 50\%$ no treino todo.
        \item Adversário na medida = definição operacional de \textbf{currículo}: dificuldade sobe com a competência.
      \end{itemize}
      \medskip
      \begin{quizbox}{}
    ...
    \begin{column}{0.48\textwidth}
      \pause
      \begin{respbox}
        Equilíbrio: $z$ quase moeda honesta — entropia máxima, $\approx 1$ bit por partida. Muito mais forte: $z \equiv -1$, nada a aprender; fraco: o mesmo, sinal trocado. Auto-jogo mantém a recompensa \alert{densa em informação}.
      \end{respbox}
      \begin{ideiabox}
        Por que RL funciona em jogos e sofre no real: jogos vêm com gerador de currículo embutido; robótica não.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Quanto vem da arquitetura?}
  \footnotesize
  Quatro redes, \emph{mesmo} conjunto fixo de partidas: política/valor \emph{separadas} (sep) ou \emph{conjuntas} (dual); \emph{conv} ou \emph{res}. Mesma busca, 5\,s por jogada.
  ...
  \medskip
  \begin{ideiabox}
    Cada mudança vale sozinha mais de \textbf{600 Elo}; juntas, mais de 1\,200. Parte do ``algoritmo'' era engenharia de rede.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Quanto vem da busca?}
  \footnotesize
  Rede do AlphaGo Zero \alert{sem nenhum lookahead} — $\argmax_a \redepol(s)_a$ — contra o sistema completo:
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        Busca = \textbf{amplificador de computação em tempo de decisão} — a mesma alavanca do \emph{test-time compute}.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Os dados humanos são necessários?}
  \footnotesize
  Mesmo laço de treino: rede iniciada por supervisionado sobre partidas humanas (KGS) vs.\ pesos aleatórios.

  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Supervisionada: \textbf{mais rápida no início}, prevê jogadas humanas melhor — o tempo todo.
        \item \emph{Tabula rasa}: ultrapassa em poucas horas, termina \textbf{mais forte}.
        \item Zero derrotou a versão do Lee Sedol \textbf{100 a 0}, 4 TPUs vs.\ 48.
        \item 3 dias: 4,9 milhões de partidas. 40 dias, 40 blocos: 29 milhões, Elo 5\,185.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertabox}
        \textbf{Prever o humano ≠ jogar bem.} Imitação carrega o teto do imitado; o auto-jogo não tem teto.
      \end{alertabox}
      \begin{ideiabox}
        Simulador perfeito + recompensa verificável é raro; sem eles (diálogo, medicina, direção), ``dispensar humanos'' \emph{não} se transfere.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Um algoritmo, três jogos}
  \footnotesize
  Removeu o restante específico do Go (simetrias do tabuleiro, torneio entre gerações); mesmo código em xadrez, shogi e Go.
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{ideiabox}
        $10^3\times$ menos posições examinadas, vitória: a rede não acelera a busca — escolhe \emph{muito melhor} o que examinar.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{alertabox}
        Condições contestadas (versão, hardware, tempo, sem livro de aberturas); superior até 10:1 em desvantagem de tempo. Informativo, não medição limpa.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{A ideia que sobrevive à mudança de domínio}
  ...
  \medskip
  \small
  Só muda o simulador; busca, PUCT, auto-jogo e perda conjunta permanecem.
\end{frame}

%% ------------------------------------------------------------
\begin{frame}{AlphaTensor: multiplicar matrizes mais rápido}
  \footnotesize
  Multiplicar $n \times n$ = decompor $\mathcal{T}_n$ em tríades de posto 1; \emph{posto} = número de multiplicações escalares. Ação = tríade; episódio termina quando o tensor residual zera.
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{ideiabox}
        $4\times4$ vale em $\mathbb{Z}_2$, não nos reais — distinção que a imprensa perdeu. Ainda assim, primeiro avanço em dois níveis em meio século.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertabox}
        Penalizar o \emph{tempo medido} na V100 rendeu 10--20\% \emph{naquele} hardware: otimalidade é relativa à máquina. Depois, Kauers e Moosbauer: $5\times5$ GF(2) a 95, busca clássica.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{AlphaDev: escrever assembly mais rápido que o humano}
  \small
  \textbf{AssemblyGame:} estado = programa assembly construído; ação = instrução; recompensa = \emph{correção} (testada contra entradas) + \emph{latência}.

  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Alvo: \texttt{sort3}, \texttt{sort4}, \texttt{sort5} — folhas de qualquer ordenação maior.
        \item Duas sequências novas (\emph{swap}, \emph{copy}): economizam uma instrução por aplicação.
        \item \texttt{VarSort4}: estrutura diferente, 29 instruções mais curto que o humano.
        \item Integrado à \texttt{libc++} do LLVM — primeira mudança na década e a primeira vinda de RL.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \end{teobox}
      \begin{ideiabox}
        Rotinas otimizadas são \emph{folhas}: melhora muito a folha, pouco o todo — mas chamadas trilhões de vezes por dia.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{MuZero e o retorno do modelo desconhecido}
  \small
  AlphaZero exige simulador perfeito; MuZero \alert{aprende} um modelo — só o suficiente para planejar.
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Dinâmica} $g$: $(s^k, a) \mapsto (s^{k+1}, r^k)$, no espaço latente.
        \item \textbf{Predição} $f$: $s^k \mapsto (\bm{p}, v)$.
      \end{itemize}
      \medskip
      MCTS roda \emph{inteiramente} no latente; nada reconstruir a observação — só política, valor e recompensa certos.
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        \textbf{Suficiente para decidir, não para prever} — modelo orientado ao valor, não gerativo; prever pixels é caro e irrelevante.
      \end{ideiabox}
...
\begin{frame}{A pergunta incômoda}
  \small
  UCT: cada nó = bandit; cota superior de confiança escolhe a ação a simular.
  ...
  \pause
  \begin{respbox}
    Bandit real: braço puxado \emph{custa}; arrependimento cumulativo é o objetivo certo. Na simulação não se perde nada — só CPU; objetivo: \alert{melhor decisão na raiz ao final do orçamento}. UCT resolve o errado.
  \end{respbox}
\end{frame}
...
\begin{frame}{Arrependimento cumulativo e arrependimento simples}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \end{teobox}
      \begin{ideiabox}
        Para $R_n$ pequeno, para-se de amostrar o aparentemente ruim — exatamente o que traria certeza sobre o melhor.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O que se deveria fazer: raciocínio metanível}
  \footnotesize
  Pergunta certa não é ``qual ação simular por mais recompensa simulada'', e sim:
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Literalmente um MDP: estados = crenças sobre valores; ações = \emph{computações} (Hay, Russell, Tolpin e Shimony, 2012).
        \item Só vale se puder \alert{mudar} a jogada; refinar o último colocado é desperdício.
        \item BRUE, SHOT, \emph{sequential halving} em árvores: superam o UCT em orçamento fixo.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        PUCT afasta-se do UCT na direção certa (por acaso histórico): decaimento $\Nvis(s,a)^{-1}$, crescimento $\sqrt{\Nvis(s)}$ — o que Grill et al.\ formalizam.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{A parede da profundidade}{Um experimento pequeno, um resultado desconfortável}
  \footnotesize
  \textbf{Árvore-armadilha} (espírito de Coquelin e Munos, 2007): em cada $d$, \emph{parar} = $0{,}9 - 0{,}4\,d/D$; \emph{seguir} até $D$ = $1{,}0$. Tentação no começo, recompensa no fim.
  \smallskip
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
    ...
    \begin{column}{0.52\textwidth}
      \begin{alertabox}
        \textbf{Duas causas:} (i) rollout alcança a folha com prob.\ $2^{-D}$; (ii) UCT retropropaga a \emph{média}, não o máximo — política mudou, viés se compõe ao subir.
      \end{alertabox}
      \begin{ideiabox}
        Coquelin e Munos: amostras \emph{exponencial em exponencial} na profundidade; consistência assintótica verdadeira e inútil.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O ajuste de $c$ não é secundário}
  \footnotesize
  Mesma árvore, $D=6$, orçamento fixo de 2\,000 simulações, variando só a constante de exploração:
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{alertabox}
        Curva \textbf{não monotônica nem suave}: $c=0{,}7$ acerta 65\%; $c=1{,}0$ falha sempre. Pouca exploração não sai da armadilha; muita contamina as médias.
      \end{alertabox}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        Explica os 94/103 Elo só com Bayesiana nos hiperparâmetros; pergunte \emph{quanto} ajuste houve — $c$ pode valer mais que o algoritmo.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{enumerate}\setlength{\itemsep}{0.4em}
        \item \textbf{A rollout foi eliminada.} Causa (i) some: folha avaliada por $\redeval$, não por partida aleatória.
        \item \textbf{O prior poda a largura.} $\approx 250$ jogadas no Go → uma dúzia examinada; ramificação efetiva despenca.
        \item \textbf{As árvores reais não são adversárias.} Em Go e xadrez, posições boas têm vizinhos bons — o gradiente existe.
        \item \textbf{O laço de treino conserta a heurística.} Onde $\redeval$ erra, o auto-jogo gera dados ali.
      \end{enumerate}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \end{teobox}
      \begin{alertabox}
        Diagnóstico: suspeite primeiro do avaliador de folha e do prior; só depois da constante de exploração ou do orçamento.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Quando a MCTS é uma boa escolha?}
  ...
  \begin{respbox}
    ...
    \textbf{(1)} MDP exato: iteração de valor devolve $\polstar$ para
    \emph{todos} os estados, mesmo preço.
    \textbf{(2)} A árvore ramifica \emph{em cada ação}; poucos estados →
    programação dinâmica viável.
    \textbf{(3)} Amostragem substitui a soma sobre $s'$; custo = orçamento,
    não $\abs{\gS}$.
  \end{respbox}
\end{frame}
...
\begin{frame}{Tratar cada nó como um bandit}
  ...
  \pause
  \begin{respbox}
    \textbf{As duas são verdadeiras} — a segunda é o ponto. Arrependimento
    simulado \alert{não é um custo}; objetivo = arrependimento \emph{simples} na
    raiz — minimizar um prejudica o outro.
  \end{respbox}
\end{frame}
...
\begin{frame}{Revisão: para que servem as cotas superiores de confiança}
  ...
  \begin{respbox}
    ...
    \textbf{Bandit}: modelo de recompensa conhecido encerra o problema.
    \textbf{MDP}: não — com $\tran{s'}{s,a}$ desconhecida, incerteza sobre
    \emph{para onde} cada ação leva. Recompensa conhecida $\neq$ valor conhecido.
  \end{respbox}
\end{frame}
...
\begin{frame}{O que levar desta aula}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{enumerate}\setlength{\itemsep}{0.35em}
        \item \textbf{Go: modelo conhecido, grande demais} — AlphaZero é planejamento com heurísticas aprendidas.
        \item \textbf{Busca = operador de melhoria de política.} Treinar para imitar a busca = iteração de política.
        \item \textbf{Auto-jogo = gerador de currículo.} Resultado incerto e, portanto, informativo.
        \item \textbf{Alavanca principal: avaliador de folha.} Rollout → rede de valor = MC → TD (viés–variância).
      \end{enumerate}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{enumerate}\setlength{\itemsep}{0.35em}\setcounter{enumi}{4}
        \item \textbf{PUCT ≠ cota de confiança.} Regularização ao prior, enfraquece com $\Nvis^{-1/2}$.
        \item \textbf{UCT resolve o problema errado.} Importa o arrependimento simples; MCTS moderna funciona apesar disso.
        \item \textbf{Busca = computação em tempo de decisão.} Mesma rede vale 2\,100 Elo a mais pensando antes.
        \item \textbf{Cuidado ao extrapolar.} Simulador perfeito + recompensa verificável é raro; ``dispensar humanos'' não transfere.
      \end{enumerate}
    \end{column}
  \end{columns}
\end{frame}
