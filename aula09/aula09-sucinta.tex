...
%%  mesmas definições e figuras do aula09.tex)
...
\begin{frame}{Retomando a aula anterior}
  ...
  \begin{respbox}[fontupper=\footnotesize]
    \textbf{Nenhuma das três.} (1) DPO dispensa o modelo explícito: a
...
\begin{frame}{Onde estamos no curso}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        Antes: \emph{``o algoritmo converge para a política ótima?''}
        Agora: \emph{``quantos erros ele comete no caminho?''}
      \end{ideiabox}
    \end{column}
  \end{columns}
  ...
  \bigskip
  \begin{alertabox}
    Convergência assintótica é critério barato: testar todas as ações um
    milhão de vezes e depois agir otimamente ``converge'' — e é inútil se
    cada teste custa um paciente, um cliente ou uma hora de robô.
  \end{alertabox}
\end{frame}
...
\begin{frame}{O que vamos construir hoje}
  ...
  \bigskip
  \begin{ideiabox}
    A matriz cenário $\times$ arcabouço $\times$ abordagem não é ponto a
    ponto: \textbf{uma mesma abordagem costuma atender vários arcabouços em
    vários cenários} — o otimismo, visto hoje em bandidos, reaparece quase
    sem alteração em MDPs.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Como julgar um algoritmo de aprendizagem}
  ...
  \begin{alertabox}
    ...
    aprendizagem} — e o que importa em ensaios clínicos, precificação,
    recomendação e robótica: cada decisão subótima é dano real, não número
    numa curva de treino.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Bandido multi-braços}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{itemize}\small
        \item Sem estado: a ação de hoje não altera o problema de amanhã.
...
\begin{frame}{Bandido, bandido contextual, MDP}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        A exploração é \emph{difícil} nos três casos, por razões diferentes:
        no bandido, basta decidir \emph{o que testar}. No MDP, é preciso
        decidir também \emph{como chegar} ao lugar onde vale a pena testar
        --- exploração profunda.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.48\textwidth}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        ...
        \item Desfecho binário: consolidou ($+1$) ou não ($0$) após seis
              semanas, por radiografia.
    ...
    \begin{column}{0.43\textwidth}
      \begin{alertabox}
        Exemplo \textbf{fictício} — os números não representam a eficácia
        real de nenhum tratamento ortopédico.
      \end{alertabox}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{alertabox}
        ...
        $a_2$, os outros dois permanecem congelados em $\Qhat = 0$: nunca
        mais são testados. A cirurgia --- de fato superior --- ficou
        eliminada por um único paciente azarado.
      \end{alertabox}
      \begin{ideiabox}
...
\begin{frame}{Decomposição: lacunas $\times$ contagens}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{ideiabox}
        Bom algoritmo: \textbf{contagens pequenas onde as lacunas são
        grandes}. O problema: as lacunas são exatamente o que não sabemos
        --- se soubéssemos $\gap_a$, não haveria nada a aprender.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{alertabox}
        ...
        \emph{ordenar} --- menos amostras onde a lacuna é grande, mais onde
        ela é pequena.
      \end{alertabox}
    \end{column}
  \end{columns}
...
\begin{frame}{Arrependimento do guloso, passo a passo}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{alertabox}
        Linhas 2 e 4: \textbf{a recompensa observada foi $+1$ e mesmo assim
        houve arrependimento} — não compara com o que aconteceu, mas com o
        que aconteceria \emph{em média} sob a melhor ação.
      \end{alertabox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        Em $a_2$ para sempre: cada passo adicional custa $0{,}05$. Ao fim
        de $T$ decisões, $\Reg_T \approx 0{,}05\,T$: \textbf{arrependimento
        linear}. A curva nunca achata.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{As duas cotas descrevem o mesmo algoritmo}
  ...
  \begin{ideiabox}
    ``Bom'' = \textbf{sublinear}: $\Reg_T / T \to 0$ — a fração de decisões
    erradas tende a zero. Explorar para sempre: linear; nunca explorar:
    também. Existe algo no meio? Sim.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O algoritmo $\epsilon$-guloso}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{alertabox}
        O preço é permanente: com $\epsilon$ fixo, uma fração $\epsilon$
        dos passos é deliberadamente subótima --- \textbf{para sempre},
        muito depois de já saber qual é o melhor braço.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{$\epsilon$-guloso no exemplo dos dedos}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \pause
      \begin{ideiabox}
        \textbf{Sim, $a_3$ voltará a ser escolhido} --- cerca de $T/30$
        pacientes receberão ``não fazer nada'', um tratamento que já
        sabemos ser péssimo.
      \end{ideiabox}
    \end{column}
  \end{columns}
...
\begin{frame}{Teste sua compreensão: arrependimento do $\epsilon$-guloso}
  ...
  \begin{respbox}
    \textbf{As duas} --- por motivos opostos, o que é o ponto da pergunta.
    $\epsilon = 0{,}1$: \emph{explora demais} — braços ruins numa fração
    fixa $\epsilon/m$ dos passos, para sempre, e
    $\Reg_T \ge \frac{\epsilon}{m}\bigl(\sum_a \gap_a\bigr) T$.
    $\epsilon = 0$: \emph{explora de menos} — é o guloso, que pode travar
...
\begin{frame}{Explorar sempre e nunca explorar falham igual}
  ...
  \begin{alertabox}
    O agendamento ótimo depende de $\gap$, que é desconhecido. Por isso o
    $\epsilon$-guloso decrescente é teoricamente correto mas praticamente
    frágil --- e por que vale a pena procurar algo que se adapte sozinho.
  \end{alertabox}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      \begin{ideiabox}[fontupper=\footnotesize]
        ...
        $\sqrt{T}$: \textbf{separar} exploração e explotação em fases
        estanques desperdiça o que se aprendeu \emph{durante} a exploração
        para abandonar cedo um braço ruim.
      \end{ideiabox}
...
\begin{frame}{Quão bom é possível ser? Limite inferior}
  \begin{itemize}
    ...
    \item Formalmente: a lacuna $\gap_a$ e a divergência
          $\KL(\mathcal{R}_a \,\|\, \mathcal{R}_{a^{*}})$.
  \end{itemize}
...
\begin{frame}{O princípio do otimismo}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{teobox}{Desfecho 1 --- acertamos}
        Braço com média alta de fato: colhemos recompensa alta. Nenhum
        arrependimento.
      \end{teobox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{teobox}{Desfecho 2 --- aprendemos}
        Média baixa: puxá-lo reduz (em esperança) sua média estimada
        \emph{e} a incerteza. O braço é rebaixado e não volta a enganar.
      \end{teobox}
    \end{column}
  \end{columns}
  ...
  \pause
  \begin{ideiabox}
    Otimismo é \textbf{autocorretivo}: só se pode estar excessivamente
    otimista por um número limitado de puxadas — cada puxada corrige o
    excesso. Essa contagem --- ``quantas vezes posso me enganar antes de
    ser desmentido?'' --- vira a demonstração da cota.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{De onde vem a largura da barra}
  ...
  \begin{ideiabox}
    ...
    $\tfrac12$-sub-gaussianas pelo lema de Hoeffding. Usar $\sigma = 1$ é
    apenas conservador: barras mais largas que o necessário, nunca mais
    estreitas.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Invertendo a desigualdade}
  Pergunta operacional: ``\emph{dado} um risco $\delta$ que aceito correr,
  qual $\varepsilon$?''
...
\begin{frame}{UCB1 no exemplo dos dedos}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{ideiabox}
        Em $t = 8$, $a_3$ é escolhido \emph{apesar} de $\Qhat(a_3) = 0$:
...
\begin{frame}{O nível de confiança $\delta$ é sutil}
  ...
  \begin{alertabox}
    Todas simultâneas com probabilidade $1 - \delta$: cada cota precisa de
    risco $\delta/(mT)$. Como $\delta$ entra como $\log(1/\delta)$, o
    preço é apenas $\log(mT)$ --- \textbf{logarítmico no número de cotas}.
    União de eventos, brutal como parece, é aceitável aqui.
  \end{alertabox}
...
\begin{frame}{Cota de arrependimento do UCB --- montagem}
  Sejam $\gap_i = \mu^{*} - \mu_i$ e $\Nc_i(n)$ as puxadas do braço $i$ até
  o passo $n$. Já sabemos:
  \[
  ...
  \vspace{-3mm}
  \begin{ideiabox}
    \textbf{No bom evento: puxadas limitadas por argumento determinístico;
    fora dele, por $n$ — probabilidade minúscula.}
  \end{ideiabox}
\end{frame}
...
\begin{frame}{A cota independente do problema}
  Cota anterior inútil com lacuna minúscula. Solução: \textbf{partir os
  braços em dois grupos} num limiar $\gap$ à escolha.
  \vspace{-0.3em}
  ...
  \begin{ideiabox}
    Piso minimax $\Omega(\sqrt{m n})$: UCB1 a um fator $\sqrt{\log n}$
    (MOSS, KL-UCB eliminam). \textbf{O mesmo algoritmo} satisfaz as duas
    cotas: adapta-se sozinho à dificuldade do problema.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Verificando as cotas nos números do exemplo}
  ...
  \vspace{-2mm}
  \begin{alertabox}[fontupper=\footnotesize]
    \textbf{Cotas do UCB piores que o desempenho observado dos métodos
    ingênuos} — não é erro: cota de \emph{pior caso} vale para todo bandido
    com estas lacunas; números do guloso, para \emph{esta} realização.
  \end{alertabox}
  \begin{ideiabox}[fontupper=\footnotesize]
...
\begin{frame}{Uma segunda família: casamento de probabilidade}
  ...
  \vspace{-3mm}
  \begin{ideiabox}
    Cada braço é escolhido com probabilidade \emph{igual à probabilidade
    posterior de ele ser o ótimo}. A exploração não é imposta por um bônus
    --- ela emerge da incerteza da posteriori, e desaparece sozinha quando
    a incerteza some.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Thompson com braços de Bernoulli}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        $\epsilon$-guloso daria $3{,}3\%$ a $a_3$ \emph{para sempre}.
        Thompson dá $6{,}6\%$ \emph{agora}, quando ainda há dúvida
        legítima, e essa fração cai sozinha à medida que a posteriori de
        $a_3$ se concentra perto de $0{,}1$.
      \end{ideiabox}
      \begin{alertabox}
        $\Prob(\text{melhor}) \ne$ ordenação das médias: $a_3$ tem média
        posterior bem menor, mas $6{,}6\%$ de chance de ser o melhor ---
