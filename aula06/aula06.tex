...
\begin{frame}{Onde estamos}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.32\textwidth}
      \begin{block}{Aula 7 --- a seguir}
        \small GAE, teoria de melhoria monotônica e aprendizagem por
        imitação.
      \end{block}
    \end{column}
  \end{columns}
  ...
  \vfill
  \begin{ideiabox}
    A Aula 5 respondeu \emph{para onde} andar. Hoje: \alert{com que precisão}
    conhecemos essa direção, e \alert{que passo} podemos dar sem cair.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Plano de hoje}
  \begin{enumerate}
    \item \textbf{Linha de base} --- por que subtrair $b(s)$ do retorno não
          muda o gradiente esperado, mas encolhe a variância; e qual é a
          melhor escolha de $b$.
    \item \textbf{Alvos alternativos ao retorno de Monte Carlo} --- de
          $G_t$ ao crítico: ator--crítico e a função vantagem.
    \item \textbf{Problemas estruturais do gradiente de política} ---
          eficiência amostral, colapso de desempenho, e distância no espaço de
          \emph{parâmetros} versus no espaço de \emph{políticas}.
    \item \textbf{Limites de desempenho relativo} --- lema da diferença de
          desempenho, a aproximação $\Lsub{\pol}(\polnovo)$ e a divergência KL.
    \item \textbf{PPO} --- penalidade KL adaptativa e objetivo recortado.
  \end{enumerate}

  \vfill
  {\footnotesize\color{rlCinza}Os itens 3--5 seguem os slides de gradientes de
...
\begin{frame}{Retomando a Aula 5}
  ...
  \pause
  \begin{respbox}
    \textbf{1 e 3.} O item 2 é falso: a direção é a do escore
    \emph{multiplicado} por $Q$, e se inverte quando o retorno é negativo. O
    item 4 é falso: só há garantia de \alert{ótimo local}.
  \end{respbox}
\end{frame}
...
\begin{frame}{REINFORCE: o ponto de partida}
  ...
  \pause
  \vspace{-0.6em}
  {\small Na forma de estimador com $m$ trajetórias:}
  \vspace{-0.4em}
  \[\textstyle
  ...
  \pause
  \begin{alertabox}
    \textbf{Não-viesado, porém muito ruidoso.} Estar \emph{certo em média} não
    é consolo se a variância é tão alta que o passo aponta para a direção
    errada na maioria dos lotes.
  \end{alertabox}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{enumerate}\small\setlength{\itemsep}{0.5em}
        ...
        \item \textbf{Escala e deslocamento do retorno.} Se todos os retornos
              valem $\approx 100$, todo o gradiente empurra ``para cima'', e o
              sinal útil é a \emph{diferença} entre eles.
    ...
    \begin{column}{0.48\textwidth}
      ...
      \end{defbox}
      \begin{ideiabox}
        Os dois primeiros são \alert{gratuitos}: reduzem variância sem viés. O
...
\begin{frame}{A linha de base}
  ...
  \vspace{-0.2em}
  \begin{itemize}\small
    \item Para \alert{qualquer} $b$ que dependa só do estado, e não da ação, o
          estimador continua não-viesado.
  ...
  \begin{ideiabox}
    ...
    base, uma ação medíocre num estado excelente é reforçada tanto quanto uma
    excelente.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Por que isso importa: um exemplo de três ações}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      {\scriptsize Reforço quase igual para as três: o sinal ($\pm 3$) está
      afogado no nível ($\approx 100$).}
    \end{column}
    \begin{column}{0.49\textwidth}
      \centering
      ...
      \end{tikzpicture}\\[0.4em]
      {\scriptsize $a_1$ é \emph{desencorajada}, $a_3$ encorajada. A média do
      passo é a mesma; o ruído é muito menor.}
    \end{column}
  \end{columns}
  ...
  \begin{ideiabox}
    ...
    trajetórias são necessárias para a média amostral chegar perto dele.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Por que a linha de base reduz a variância}
  \vspace{-0.4em}
  {\small
  A variância se decompõe aproximadamente por passo de tempo; concentre-se em um
  termo. Como a esperança \alert{não depende de $b$} (acabamos de provar),
  minimizar a variância é minimizar só o momento de segunda ordem:
  \vspace{-0.3em}
  \[
...
\begin{frame}{Algoritmo de gradiente de política ``baunilha''}
  ...
  \vspace{-0.2em}
  \begin{ideiabox}
    Já é um método de \alert{dois modelos} --- um para a política, outro para o
    valor: estamos a um passo do ator--crítico. Note que $b$ é ajustada com a
    \emph{mesma} amostra que gera o gradiente; prático, mas correlaciona os dois.
  \end{ideiabox}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{itemize}\small\setlength{\itemsep}{0.3ex}
        \item \textbf{Média do lote:} $b = \frac{1}{N}\sum G_t$. Constante, trivial, e já ajuda muito.
        \item \textbf{Regressão sobre atributos:} $b(s) = w^\top x(s)$.
        \item \textbf{Rede neural} treinada por regressão sobre $G_t$ --- o \emph{crítico}.
      \end{itemize}
      \vspace{0.3em}
      \begin{alertabox}
        \small Normalizar as vantagens do lote
        ($\Ahat \leftarrow (\Ahat - \bar{\Ahat})/\sigma_{\Ahat}$) é padrão em
        implementações --- e \emph{introduz} um viés pequeno.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Escolhendo o alvo}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}\small
        \item $G_t^i$ é não-viesado para $\Vpi(s_t)$ --- mas é uma amostra
              \emph{única} de uma soma de muitas variáveis aleatórias.
        \item Reduza a variância \alert{introduzindo viés}, via
              \emph{bootstrapping} e aproximação de função.
        \item É o mesmo dilema de TD versus MC (Aula 3), agora aplicado ao
              alvo do gradiente de política.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{O laço ator--crítico}
  ...
  \begin{alertabox}
    ...
    um sinal disponível \alert{no passo seguinte} --- mas, se ele está errado,
    o ator otimiza a coisa errada. Os dois aprendem ao mesmo tempo, e a
    estabilidade dessa dança é assunto de metade da literatura da área.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Do retorno à vantagem}
  ...
  \]
  \pause\vspace{-0.5em}
  E com a linha de base sendo uma estimativa de $V$, a expressão colapsa na
  \alert{função vantagem}:
  \vspace{-0.3em}
  \[
    \nabla_\theta \E_\tau[\rec] \approx \E_\tau\!\left[\sum_{t=0}^{T-1}
      \nabla_\theta \log \pol(a_t\mid s_t;\theta)\,
...
\begin{frame}{O catálogo de alvos}
  ...
  \begin{ideiabox}
    Descer nesta tabela é caminhar do \emph{Monte Carlo} para o \emph{TD}. A
    última linha interpola entre as duas com um só parâmetro $\lambda$ ---
...
\begin{frame}{Teste de compreensão --- linha de base e vantagem}
  ...
  \pause
  \begin{respbox}
    \textbf{2 e 3.} O item 1 é falso: há viés assim que $V_w \ne \Vpi$ --- é o
    preço que pagamos. O item 4 é falso: a prova de não-viés exige tirar $b$
    de dentro de $\sum_a$.
  \end{respbox}
\end{frame}
...
\begin{frame}{Limitação 1 --- eficiência amostral}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{itemize}
        \item Cada lote de dados é \alert{descartado imediatamente} após um
              único passo de gradiente.
        \item Por quê? O gradiente de política é uma esperança
              \emph{sob a política atual} --- é \textbf{on-policy} por
              construção. Assim que $\theta$ muda, os dados envelhecem.
      \end{itemize}
    ...
    \begin{column}{0.44\textwidth}
      ...
      \end{alertabox}
      {\scriptsize\color{rlCinza}É a pergunta que o PPO responde no fim da aula.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Limitação 2 --- o tamanho do passo}
  ...
  \vspace{-0.2em}
  \begin{alertabox}
    \textbf{Grande demais:} colapso --- e o colapso é \emph{auto-reforçado},
    porque a política ruim coleta dados ruins. \textbf{Pequeno demais:}
    progresso lento. E o tamanho ``certo'' \alert{muda com $\theta$}: nenhuma
    constante serve para a corrida inteira. Normalizar vantagens e usar Adam
    ajuda --- mas resolve?
  \end{alertabox}
\end{frame}
...
\begin{frame}{Limitação 3 --- o problema é mais do que o passo}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.51\textwidth}
      \begin{ideiabox}
        \small \textbf{A pergunta que organiza o resto da aula:} como atualizar
        de modo a \alert{nunca mudar a política mais do que pretendíamos}?
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O que queremos de uma regra de atualização}
  \begin{itemize}\small
    \item Usar as trajetórias da política \alert{mais recente} com a maior
          eficiência possível.
    \item Dar passos que respeitem distância no \alert{espaço de políticas},
          não no espaço de parâmetros.
  \end{itemize}
  \vspace{-0.2em}
  ...
  \begin{ideiabox}
    ...
    Toda a teoria desta aula é uma tentativa de tornar essa expressão
    computável.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{A distribuição descontada de estados futuros}
  ...
  \pause
  \vspace{-0.5em}
  {\small Substituindo no lema e reescrevendo a esperança sobre ações por
  \alert{amostragem por importância}:}
  \vspace{-0.4em}
  \[
    J(\polnovo) - J(\pol)
    = \frac{1}{1-\gamma}\E_{\substack{s\sim\dpolnovo\\ a\sim\polnovo}}\!\left[\Adv^{\pol}(s,a)\right]
...
\begin{frame}{Uma aproximação útil}
  ...
  \vspace{-0.3em}
  \begin{ideiabox}
    O erro cometido ao trocar $\dpolnovo$ por $\dpol$ é controlado pela
    \alert{divergência KL média entre as políticas}. Se elas são próximas em
    KL, a aproximação é boa --- e $\Lsub{\pol}(\polnovo)$ vira um
    \emph{substituto} legítimo do objetivo verdadeiro.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Divergência de Kullback--Leibler}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \vspace{0.3em}
      \begin{ideiabox}
        \small É a régua certa aqui porque mede diferença \emph{entre
        distribuições}, e não entre os parâmetros que as geram --- exatamente
        a distinção do slide da sigmoide.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O que ganhamos com a aproximação}
  ...
  \begin{itemize}\small
    \item Tudo do lado direito é estimável com trajetórias amostradas da
          política \alert{antiga} $\pol$. Podemos otimizar $\polnovo$ sem
          coletar dados novos --- isto é, dar \alert{vários passos de gradiente
          por lote}.
    \item É parente da amostragem por importância, mas com uma diferença
          crucial: o peso depende \emph{só do passo atual}, e não do histórico
          inteiro. Não há produto de $t$ razões, logo os pesos não explodem nem
          desaparecem.
  \end{itemize}
  ...
  \begin{alertabox}
    ...
    verdadeiro \alert{enquanto $\polnovo$ ficar perto de $\pol$ em KL}. Otimizar
    $\Lsub{\pol}$ sem restrição é otimizar um modelo fora de sua região de
    validade.
  \end{alertabox}
\end{frame}
...
\begin{frame}{A região de confiança}
  ...
  \vspace{-0.2em}
  \begin{ideiabox}
    Maximizar o substituto \emph{dentro} da bola de KL é a ideia por trás de
    TRPO e, de forma aproximada e barata, do PPO. Fora da bola, o substituto
    deixa de ser informativo sobre o objetivo real.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Teste de compreensão --- o substituto}
  ...
  \begin{respbox}
    ...
    Aula 5 --- $\Lsub{\pol}$ \emph{estende} aquele gradiente. O item 3 é
    falso: o limite vale sempre, mas a folga fica grande demais para informar
    --- daí o item 4 também ser falso.
  \end{respbox}
\end{frame}
...
\begin{frame}{PPO com penalidade KL adaptativa}
  ...
  \vspace{-0.2em}
  \begin{ideiabox}
    O valor inicial de $\beta$ \alert{não importa muito} --- ele se adapta
    rápido. Algumas iterações violam a restrição; a maioria não. E o
    \alert{$K$} é o ponto central: é aqui que finalmente damos vários passos
    por lote.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{PPO com objetivo recortado}
  ...
  \begin{itemize}\small
    ...
    \item O $\min$ com a versão recortada torna o objetivo um
          \alert{limite inferior pessimista} do substituto --- nunca
          otimista quanto ao ganho de se afastar.
  \end{itemize}
\end{frame}
...
\begin{frame}{A geometria do recorte}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      {\scriptsize O ganho \emph{satura} em $1+\epsilon$: não há prêmio por
    ...
    \begin{column}{0.49\textwidth}
      ...
      \begin{tikzpicture}
        ...
        \end{axis}
      \end{tikzpicture}\\[-0.2em]
      {\scriptsize A punição \emph{não} satura à direita: uma ação ruim que
      ficou muito mais provável continua sendo penalizada.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Por que o mínimo, e por que a assimetria}
  ...
  \begin{ideiabox}
    ...
    pessimista}: o recorte remove o incentivo para se afastar, mas
    \emph{nunca} remove o incentivo para \emph{voltar}. Se um passo anterior
    empurrou uma ação ruim para $r \gg 1$, o gradiente ainda existe para
    corrigi-lo.
  \end{ideiabox}

  \vspace{-0.2em}
  \begin{alertabox}
    O recorte limita a \emph{razão}, não a divergência KL. Não há garantia
    formal de melhoria monotônica --- é uma heurística barata inspirada na
    teoria da seção anterior.
  \end{alertabox}
\end{frame}
...
\begin{frame}{PPO recortado: o algoritmo completo}
  ...
  \vspace{-0.2em}
  \begin{ideiabox}
    Três termos na perda: o objetivo recortado do \alert{ator}; o erro
    quadrático do \alert{crítico} ($L^V$); e um bônus de \alert{entropia}, que
    impede a política de virar determinística cedo demais.
  \end{ideiabox}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{ideiabox}
        ...
        a \alert{fração recortada}. Se a KL dispara, reduza $K$ ou a taxa de
        aprendizado.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{O que você deve saber ao sair daqui}
  \begin{enumerate}\small\setlength{\itemsep}{0.35em}
    \item \textbf{Demonstrar} que uma linha de base $b(s)$ não introduz viés, e
          apontar exatamente onde a demonstração quebraria se $b$ dependesse da
          ação.
