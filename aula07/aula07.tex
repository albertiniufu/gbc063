...
%%  Compilar com XeLaTeX (2x) — latexmk -xelatex aula07.tex
%% ============================================================
...
\begin{frame}{Onde estamos}{Da estimativa do gradiente ao tamanho do passo}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    \item \textbf{Aulas 2--3.} Com um modelo do mundo, calcular $\Vstar$ e $\polstar$
      por programação dinâmica.
    \item \textbf{Aula 4.} Sem modelo, aprender $\Qhat(s,a;w)$ a partir de amostras.
    \item \textbf{Aulas 5--6.} Parametrizar a política e subir o gradiente de
      $J(\polth)$; medir distância entre políticas no espaço de \emph{distribuições}.
    \item \textbf{Hoje, primeira metade.} Fechar a família: como reaproveitar dados
      de uma política velha (\alert{amostragem por importância}), como o
      \alert{PPO} torna isso barato, como o \alert{GAE} equilibra viés e variância
      na vantagem, e \emph{por que} tudo isso funciona (teoria da melhoria monotônica).
    \item \textbf{Hoje, segunda metade.} E se não houvermos recompensa alguma, mas
      sim \alert{demonstrações}? Clonagem de comportamento, DAgger, RL inverso.
  \end{itemize}

  \begin{ideiabox}
    As duas metades se encontram no fim: os métodos que hoje ajustam modelos de
    linguagem juntam PPO (primeira metade) com uma recompensa \emph{aprendida} a
    partir de preferências humanas (segunda metade).
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Plano de hoje}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.5ex}
    \item \textbf{Amostragem por importância} --- a ferramenta que permite avaliar
      uma política nova com dados de outra, e por que ela sozinha não basta.
    ...
    \item \textbf{Estimador generalizado de vantagem (GAE)} --- um botão único,
      $\lambda$, que percorre todo o espectro entre TD(0) e Monte Carlo.
    \item \textbf{Teoria da melhoria monotônica} --- o limite de desempenho relativo
      e o argumento de \emph{majorizar--maximizar} que justifica TRPO e PPO.
    ...
    \item \textbf{RL inverso} --- recompensa linear em atributos, casamento de
      expectativas, e o princípio da máxima entropia.
  \end{enumerate}
\end{frame}
...
\begin{frame}{O que ainda incomoda no gradiente de política}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item \textbf{Eficiência amostral ruim.} O estimador é \emph{on-policy}: cada
      lote de trajetórias serve a \emph{uma} atualização e é descartado.
    \item \textbf{Distância em $\thv$ $\neq$ distância em $\pol$.} O passo é dado
      no espaço de parâmetros, mas o que estraga (ou salva) o aprendizado é o
      deslocamento no espaço de \emph{políticas}.
  \end{itemize}
...
\begin{frame}{O mesmo passo em $\thv$, dois deslocamentos muito diferentes}
  ...
  Desloque $\mu$ da mesma quantidade $\Delta\mu = 0{,}5$ nos dois casos:
  ...
  \vspace{0.2em}
  \begin{alertabox}
    Um único passo $\alpha$ não pode estar certo nos dois regimes. É por isso que
    a taxa de aprendizado do gradiente de política é notoriamente difícil de
    ajustar --- e por que faz sentido controlar o passo pela \alert{divergência KL}
    entre as políticas, não pela norma em $\thv$.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Amostragem por importância}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item A identidade é exata --- o estimador é \textbf{não-viesado} para qualquer $Q$
      admissível.
    \item O problema nunca é o viés. É a \textbf{variância}.
  \end{itemize}

  \begin{ideiabox}
    Intuição: reponderamos cada amostra pelo quanto ela era \emph{mais provável sob
    $P$ do que sob $Q$}. Se $Q$ raramente visita uma região onde $P f$ é grande,
    as poucas amostras que caem lá carregam um peso enorme --- e o estimador passa
    a depender delas.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{A variância do estimador de importância}
  ...
  \begin{alertabox}
    O termo destacado é o vilão. Ele pesa $f(x)^2$ pela razão $P/Q$: se $P/Q$ for
    grande exatamente onde $f$ é grande, a variância explode --- e pode ser
    \emph{infinita} mesmo com $P$ e $Q$ perfeitamente bem-comportadas.
  \end{alertabox}

  \begin{itemize}
    \item Diagnóstico prático: o \emph{tamanho amostral efetivo}
      $\mathrm{ESS} = (\sum_i \omega_i)^2 / \sum_i \omega_i^2$, com $\omega_i = P(x_i)/Q(x_i)$.
      Se $\mathrm{ESS} \ll N$, poucas amostras estão fazendo todo o trabalho.
  \end{itemize}
\end{frame}
...
\begin{frame}{Amostragem por importância no gradiente de política}
  ...
  \begin{ideiabox}
    É o mesmo cancelamento da Aula 5: modelo do mundo desconhecido, mas irrelevante.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O produto é uma bomba-relógio}
  ...
  \begin{column}{0.52\textwidth}
    Suponha que em cada passo a razão valha $1{,}02$ --- políticas praticamente
    indistinguíveis, $2\%$ de diferença por ação.
  ...
  \begin{column}{0.46\textwidth}
    \begin{alertabox}
      Diferenças minúsculas \emph{multiplicadas} ao longo do horizonte viram uma
      diferença enorme. Na direção oposta, o peso vai a zero e a trajetória some
      do estimador.
    \end{alertabox}

    \begin{ideiabox}
      \textbf{A pergunta central desta primeira metade da aula:}
      como aproveitar de forma eficiente os dados que já temos da política velha,
      sem pagar o preço da razão de importância sobre trajetórias inteiras?
    \end{ideiabox}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \begin{respbox}
    Porque \textbf{não-viesado não é o mesmo que útil}. À medida que $\thv$ se afasta
    de $\thv_0$, a variância cresce até que o tamanho amostral efetivo caia para
    ordem de uma ou duas trajetórias: o estimador continua correto \emph{em
    esperança}, mas cada realização é lixo. Os métodos de hoje atacam isso por dois
    lados --- \alert{limitar o quanto $\thv$ pode se afastar} (região de confiança,
    penalidade KL, recorte) e \alert{substituir a razão de trajetórias por uma razão
    de um único passo}.
  \end{respbox}
\end{frame}
...
\begin{frame}{O objetivo substituto}
  ...
  \vspace{-0.3em}
  \begin{alertabox}
    O incômodo: os estados vêm de $d^{\polnovo}$, política que ainda não executamos.
  \end{alertabox}
...
\begin{frame}{PPO: duas maneiras de manter $\polnovo$ perto de $\pol$}
  ...
  \begin{teobox}{Variante 1 --- penalidade KL adaptativa}
    ...
    \]
    O coeficiente $\beta_k$ é ajustado entre iterações: se a KL medida ficou acima do
    alvo, aumenta-se $\beta$; se ficou abaixo, diminui-se --- restrição KL aproximada.
  \end{teobox}
...
\begin{frame}{Lendo os dois ramos}
  ...
  \begin{column}{0.49\textwidth}
    ...
    \smallskip
    {\small
    \textbf{Se $\Adv>0$} (ação boa): queremos aumentar $r$. O objetivo cresce
    linearmente até $r = 1+\epsilon$ e \emph{satura}. Passado esse ponto o gradiente
    é zero: não há mais incentivo em empurrar a probabilidade da ação para cima.
    \par\smallskip
    \textbf{Se $\Adv<0$} (ação ruim): queremos diminuir $r$. O objetivo é constante
    para $r < 1-\epsilon$ --- já reduzimos o suficiente, pare. Mas do outro lado ele
    \emph{decresce sem limite}.}
  \end{column}
  \begin{column}{0.49\textwidth}
    \begin{alertabox}
      Duas leituras que costumam escapar:
      \begin{itemize}\setlength{\itemsep}{0.15ex}
        \item O recorte remove o \emph{incentivo} de ir mais longe; ele \textbf{não
          impede} que $r$ fique grande. Se a primeira época de otimização já jogou
          $r$ para $3$, o recorte não a traz de volta.
        \item Para $\Adv<0$ o objetivo é ilimitado inferiormente --- uma ação ruim
          pode ser punida arbitrariamente. É uma assimetria real do PPO.
      \end{itemize}
    \end{alertabox}
    \begin{ideiabox}
      O $\min$ faz de $\Lclip$ um limitante \emph{pessimista} (inferior) do
      substituto não recortado: nunca somos otimistas sobre o ganho de nos afastar.
    \end{ideiabox}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{PPO recortado: o algoritmo}
  ...
  \begin{ideiabox}
    O ganho essencial está no laço interno: \textbf{várias atualizações de gradiente
    com o mesmo lote}. É de onde vem a eficiência amostral --- e é exatamente por
    isso que precisamos do recorte, porque a partir da segunda época os dados já são
    \emph{off-policy}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{PPO na prática}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{0.2ex}
      ...
      \item Só primeiras derivadas --- sem inverter matriz de Fisher, sem gradiente
        conjugado (diferentemente do TRPO).
      \item Poucas linhas a mais que o REINFORCE.
      \item Robusto o bastante para virar padrão --- inclusive no ajuste de modelos
        de linguagem por RLHF.
    \end{itemize}
  \end{column}
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{0.2ex}
      ...
      \item A melhoria monotônica é \emph{aproximada}: o recorte é uma heurística
        inspirada na teoria, não uma implementação dela.
  ...
  \begin{alertabox}
    A ideia de \emph{atualização conservadora de política} --- não confie no
    substituto longe do ponto onde ele foi construído --- é anterior ao PPO e
    remonta ao início dos anos 2000 (\emph{conservative policy iteration},
    Kakade e Langford). O PPO é a encarnação barata dessa ideia.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Recapitulação: estimadores de $n$ passos e o erro TD}
  ...
  \begin{ideiabox}
    Reparação central: $\Ahat^{(1)}_t = \delta^V_t$. Todo o resto desta seção é
    descobrir que os estimadores de $n$ passos são \emph{somas de $\delta$'s}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Estimador generalizado de vantagem (GAE)}
  ...
  \vspace{-0.4em}
  \begin{ideiabox}
    Um desconto exponencial de razão $\gamma\lambda$ aplicado aos erros TD futuros.
...
\begin{frame}{Os dois extremos}
  ...
  \begin{column}{0.48\textwidth}
    \begin{teobox}{$\lambda = 0$}
      ...
      A vantagem de \textbf{TD(0)}. Usa uma recompensa real e o resto vem de $V$.
  ...
  \begin{column}{0.48\textwidth}
    \begin{teobox}{$\lambda = 1$}
      ...
      A vantagem de \textbf{Monte Carlo}. Os $V$ internos telescopam e somem.
      \begin{itemize}\setlength{\itemsep}{0.1ex}
  ...
  \begin{alertabox}
    ...
    $\E[\Ahat_t] = \Qpi(s_t,a_t) - V(s_t)$, igual a $\Advpi$ só se $V=\Vpi$. Não-viesado,
    para qualquer $V$, é o \textbf{gradiente}: a diferença é função só do estado.
  \end{alertabox}
\end{frame}
...
\begin{frame}{$\gamma$ e $\lambda$ fazem coisas diferentes}
  ...
  \begin{alertabox}
    Os dois aparecem multiplicados ($\gamma\lambda$), o que os faz parecer
    intercambiáveis. Não são: $\gamma$ está no problema, $\lambda$ está no estimador.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Medindo o compromisso}
  ...
  \begin{alertabox}
    Olhe a célula em negrito. Sem crítico e com $\lambda=0$, o viés $+0{,}478$ é
    \emph{maior em módulo} que o próprio gradiente $-0{,}2075$: em média o estimador
    aponta para o \textbf{lado errado}. Nenhuma quantidade de amostras corrige isso.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Por que $\lambda \approx 0{,}95$, e não $\lambda = 0$}
  ...
  \begin{ideiabox}
    \textbf{O $\lambda$ ótimo cresce com o tamanho do lote.} Com poucas trajetórias a
    variância domina e vale a pena aceitar viés ($\lambda$ pequeno); com lotes
    grandes a variância já está controlada e o viés passa a ser o gargalo. Os
    valores usuais na literatura ($\lambda \in [0{,}9;\,0{,}99]$) são a resposta certa
    para o regime em que o PPO opera: milhares de passos por atualização.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{GAE truncado: como o PPO realmente calcula}
  ...
  \begin{itemize}\setlength{\itemsep}{0.2ex}
    ...
    \item O truncamento introduz viés (a cauda além de $T$ some), pequeno se
      $(\gamma\lambda)^{T}\ll1$. O indicador $\mathrm{fim}_t$ zera o \emph{bootstrapping}
      em estados terminais --- erro clássico de implementação.
  \end{itemize}
\end{frame}
...
\begin{frame}{Voltando ao limite entre duas políticas}
  ...
  Da aula passada: usamos $\dpi$ no lugar de $d^{\polnovo}$. \emph{Por quê?}
  Porque não temos amostras de $d^{\polnovo}$. Mas a troca custa alguma coisa ---
  e a teoria diz exatamente quanto.
  ...
  \begin{ideiabox}
    Leitura em uma frase: \emph{o substituto é confiável exatamente na medida em que
    as duas políticas estão próximas em KL}. A aproximação não é ruim --- ela é
    ruim \emph{longe}, com erro controlado.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{De um limite a um algoritmo}
  ...
  Tirando o valor absoluto e ficando com o lado que interessa:
  \[
    J(\polnovo) - J(\pol) \;\ge\;
  ...
  \begin{column}{0.52\textwidth}
    \begin{itemize}\setlength{\itemsep}{0.3ex}
      \item Se \textbf{maximizarmos o lado direito} em $\polnovo$, garantimos
        melhoria do lado esquerdo.
      \item É um algoritmo de \alert{majorizar--maximizar} (MM) aplicado ao
        objetivo verdadeiro.
      \item E o milagre prático: \emph{os dois} termos do lado direito são
        estimáveis com amostras de $\pol$ --- a política que já rodamos.
    \end{itemize}
  \end{column}
  \begin{column}{0.46\textwidth}
    \begin{center}
...
\begin{frame}{A garantia de melhoria}
  ...
  \begin{ideiabox}
    A demonstração sobrevive se a otimização for restrita a uma classe parametrizada
    $\Pi_{\thv}$, desde que $\polk \in \Pi_{\thv}$ --- por isso vale para redes neurais.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Por que ninguém usa esse algoritmo literalmente}
  ...
  \begin{alertabox}
    O $C$ que a teoria fornece cresce como $\gamma/(1-\gamma)^2$. Com
    $\gamma = 0{,}99$ isso é da ordem de $10^4$: a penalidade domina o substituto e
    os passos ficam \emph{absurdamente pequenos}. A garantia é verdadeira e inútil.
  \end{alertabox}

  \begin{ideiabox}
    Duas saídas, e as duas abandonam a constante teórica: transformar a penalidade em
    \textbf{restrição} com um raio $\delta$ escolhido à mão (região de confiança,
    TRPO), ou \textbf{ajustar} o coeficiente da penalidade por realimentação (PPO).
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Três aproximações do mesmo algoritmo idealizado}
  ...
  \begin{ideiabox}
    A teoria da melhoria monotônica não é \emph{executada} por nenhum dos três. Ela
    explica \emph{por que} vale a pena restringir o passo --- e é essa explicação, não
    a fórmula, que sobreviveu à prática.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \pause
  \begin{respbox}
    \textbf{(a)} e \textbf{(d)} são verdadeiras. \textbf{(b)} é falsa: a otimização é
    não-convexa e converge a ótimos locais. \textbf{(c)} é falsa e vale insistir: uma
    política \emph{determinística} nunca toma as demais ações --- sem exploração, o
    gradiente não enxerga as alternativas. \textbf{(d)} é a motivação desta seção.
  \end{respbox}
\end{frame}
...
\begin{frame}{Balanço da primeira metade}
  ...
  \begin{alertabox}
    Tudo isso pressupõe que existe uma \textbf{função de recompensa}. E se não
    existir --- se tudo o que temos for alguém fazendo a tarefa corretamente?
  \end{alertabox}
\end{frame}
...
\begin{frame}{Aprender a partir de decisões e resultados passados}
  ...
  Em muitos domínios já \emph{existem} políticas de decisão muito boas --- executadas
  por pessoas --- e o que queremos é automatizá-las.

  \begin{columns}[T]
  \begin{column}{0.49\textwidth}
    \begin{teobox}{Ideia 1 --- humano fornece a recompensa}
      A pessoa avalia as decisões que o algoritmo de RL toma.
      \begin{itemize}\setlength{\itemsep}{0.1ex}
        \item[\textcolor{rlVerde!70!black}{$+$}] Supervisão simples e barata por episódio.
        \item[\textcolor{rlVermelho}{$-$}] Complexidade amostral altíssima: o agente
          precisa de milhões de episódios, e cada um precisa ser avaliado.
      \end{itemize}
    \end{teobox}
  \end{column}
  \begin{column}{0.49\textwidth}
    \begin{teobox}{Ideia 2 --- aprendizagem por imitação}
      ...
      \begin{itemize}\setlength{\itemsep}{0.1ex}
        ...
        \item[\textcolor{rlVermelho}{$-$}] Como generalizar para situações que o
  ...
  \begin{ideiabox}
    A escolha é frequentemente sobre \emph{qual canal de comunicação é mais fácil
    para o humano}. Especificar uma recompensa que produza direção defensiva é
    difícil; dirigir defensivamente por vinte minutos é fácil.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Recompensas densas e o problema de especificá-las}
  ...
  Recompensas \textbf{densas no tempo} guiam o agente de perto e tornam o
  aprendizado muito mais rápido que uma recompensa terminal esparsa. Como obtê-las?

  \begin{columns}[T]
  \begin{column}{0.49\textwidth}
    \begin{alertabox}
      \textbf{Projetá-las à mão} (\emph{reward shaping}) costuma ser frágil. O agente
      otimiza o que foi escrito, não o que se queria: uma penalidade por tempo mal
      calibrada produz um robô que trava, um bônus por velocidade produz um robô que
      capota.
    \end{alertabox}
  \end{column}
  \begin{column}{0.49\textwidth}
    \begin{ideiabox}
      \textbf{Especificá-las implicitamente por demonstrações.} Em vez de escrever
      $\rec$, mostre trajetórias boas e deixe o algoritmo inferir que função de
      recompensa as tornaria ótimas.
...
\begin{frame}{Formulação do problema}
  ...
  Três perguntas distintas, três famílias de métodos:
  ...
  \begin{ideiabox}
    A terceira é a mais ambiciosa: com a recompensa correta em mãos, a política
    resultante pode \emph{superar} o especialista --- o que a clonagem, por
    construção, nunca faz.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{RL direto e RL inverso}
  ...
  \begin{alertabox}
    O problema direto (a) é bem-posto: dado $\rec$, existe $\Vstar$ único.
    O inverso (b) é \textbf{mal-posto}. Voltaremos a isso --- é a dificuldade
    central de todo o RL inverso.
  \end{alertabox}

  \begin{ideiabox}
    Por que se dar ao trabalho de recuperar $\rec$ em vez de copiar a política?
    Porque a recompensa é a descrição \emph{mais transferível} de uma tarefa: ela
    continua válida se a dinâmica mudar, se o corpo do robô mudar, se o horizonte
    mudar. A política, não.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Clonagem de comportamento}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \begin{itemize}\setlength{\itemsep}{0.15ex}
      \item \textbf{ALVINN} (Pomerleau, NIPS 1989): rede rasa mapeando imagem de
        câmera em ângulo de direção; dirigiu um veículo em vias reais.
  ...
  \begin{column}{0.48\textwidth}
    \begin{ideiabox}
      ...
      (\emph{BC-RNN}), a clonagem é uma linha de base forte em manipulação robótica
      a partir de demonstrações humanas \emph{offline} (Mandlekar et al., CoRL 2021).
    \end{ideiabox}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{O problema: erros que se compõem}
  ...
  \begin{alertabox}
    O aprendizado supervisionado supõe pares $(s,a)$ \textbf{i.i.d.}\ e ignora a
    estrutura temporal. Mas em um MDP o \emph{próprio erro} muda a distribuição dos
    estados seguintes: cada desvio leva o agente para uma região onde ele foi menos
    treinado, onde erra mais, e assim por diante.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Descasamento de distribuições, dito com precisão}
  ...
  \begin{ideiabox}
    É a patologia do RL \emph{off-policy} por outro ângulo: os dados vêm de uma
    política e a avaliação ocorre sob outra. Lá a resposta foi reponderar; aqui será
    \emph{consertar os dados}.
  \end{ideiabox}
...
\begin{frame}{DAgger: agregação de conjuntos de dados}
  ...
  \begin{ideiabox}
    A ideia em uma frase: \textbf{obtenha rótulos do especialista nos estados que a
    \emph{sua} política visita}, e não apenas nos que o especialista visitaria.
  \end{ideiabox}
  ...
  \begin{itemize}\setlength{\itemsep}{0.2ex}
    \item Como o conjunto agregado cobre os estados induzidos pela própria política,
      o treino e o teste passam a compartilhar a distribuição.
    \item Análise via \emph{aprendizado online sem arrependimento}: o erro volta a
...
\begin{frame}{DAgger: o preço}
  ...
  \begin{alertabox}
    \textbf{Limitação central:} o especialista precisa estar \emph{disponível
    online} para rotular estados arbitrários --- inclusive estados esquisitos, nos
    quais ele jamais estaria, e nos quais talvez nem saiba o que faria. Rotular
    quadro a quadro é caro, cansativo e, em domínios como voo ou cirurgia,
    impraticável em tempo real.
  \end{alertabox}

  \begin{ideiabox}
    Duas saídas usuais: rotulação \emph{offline} em lote (grava-se a execução e o
    especialista rotula depois) e \textbf{HG-DAgger}, em que o humano só intervém
    quando julga necessário --- o que também dá um sinal de onde a política é
    insegura.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Da otimalidade do especialista a uma condição sobre $\wv$}
  ...
  Se as demonstrações vêm da política \emph{ótima} $\polE$ para a recompensa
  verdadeira $\rec^{*} = \wv^{*\top}\feat$, então por definição de otimalidade
  ...
  \begin{teobox}{Condição de identificação}
    ...
    \[
      ...
      \qquad \forall\, \pol \ne \polE .
    \]
    Ou seja: um vetor de pesos sob o qual o especialista bate todo mundo.
  \end{teobox}

  \begin{alertabox}
    Duas dificuldades imediatas. \emph{(i)} O conjunto de restrições é enorme --- uma
    por política. \emph{(ii)} $\wv = 0$ satisfaz todas elas com igualdade. A
    formulação, como está, é vazia.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Casamento de expectativas de atributos}
  ...
  E se, em vez de recuperar $\wv$, buscássemos direto uma política que \emph{se comporte} como o especialista?
  ...
  \begin{ideiabox}
    Garantia forte: se as expectativas de atributos casam, a política é quase tão boa
    quanto a do especialista \textbf{para qualquer recompensa linear nesses atributos}
    --- inclusive a verdadeira. Contornamos a ambiguidade em vez de resolvê-la.
  \end{ideiabox}

  \begin{alertabox}
    O peso da hipótese migrou para $\feat$: se um atributo relevante ficou de fora,
    nenhuma garantia o alcança.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Aprendizado por aprendizagem: iteração de margem máxima}
  ...
  \begin{ideiabox}
    É uma máquina de vetores de suporte alternando com um planejador: a cada
    rodada, procura-se a recompensa que mais \emph{separa} o especialista das
    políticas já geradas, e então a melhor política para essa recompensa. Quando
    ninguém mais consegue separá-los, as expectativas casaram.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \pause
  \begin{respbox}
    \textbf{(b)} --- e mais: existe um conjunto \textbf{infinito}. Dois argumentos
    imediatos: \emph{(i)} $\rec \equiv 0$ torna \emph{toda} política ótima, inclusive
    a do especialista; \emph{(ii)} escalar $\rec$ por qualquer $c>0$, ou somar uma
    constante, preserva a ordenação das políticas. Mais geralmente, transformações
    do tipo $\rec'(s,a,s') = \rec(s,a,s') + \gamma\Phi(s') - \Phi(s)$ preservam a
    política ótima para qualquer $\Phi$ (\emph{potential-based shaping}, Ng, Harada
    e Russell, 1999).
  \end{respbox}
\end{frame}
...
\begin{frame}{Contagens de atributos por trajetória}
  ...
  \begin{alertabox}
    ...
    desconto. É a do artigo de Ziebart et al.\ (2008).
  \end{alertabox}

  \begin{ideiabox}
    Reformulação-chave: em um MDP determinístico de horizonte $H$ e com recompensa
    linear, uma política fica \emph{completamente especificada} por sua distribuição
...
\begin{frame}{Princípio da máxima entropia}
  ...
  \begin{teobox}{Problema variacional (Ziebart et al., 2008)}
    Entre todas as distribuições sobre trajetórias que reproduzem as contagens de
    atributos observadas, escolha a de \textbf{maior entropia}:
    \[
  ...
  \begin{ideiabox}
    ``Nenhuma preferência além do que os dados exigem.'' Qualquer distribuição de
    entropia menor afirma sobre o especialista algo que as demonstrações não sustentam.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{MDPs estocásticos}
  ...
  Em geral a dinâmica não é determinística, e a distribuição sobre caminhos passa a
  depender \emph{também} do ambiente, não só dos pesos:
  \[
  ...
  \begin{alertabox}
    Dois efeitos se misturam: um caminho pode ser frequente porque o especialista o
    \emph{quer} (peso $\wv$) ou porque a dinâmica o \emph{empurra} para lá
    (termo $\prod P$). Separar os dois é o trabalho do modelo de transição --- e é
    exatamente aqui que ele se torna indispensável.
  \end{alertabox}

  \begin{ideiabox}
    Um caso famoso: se o especialista quase sempre acaba no mesmo lugar, isso pode
    revelar uma preferência forte, ou apenas que todos os caminhos levam ali. Sem
    saber a dinâmica, as duas explicações são indistinguíveis.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Aprendendo $\wv$}
  ...
  \begin{ideiabox}
    O gradiente é a diferença entre \emph{o que o especialista fez} e \emph{o que o
    modelo atual faria}: se o modelo já reproduz as contagens observadas, ele é nulo.
    É a forma canônica de todo modelo de máxima entropia.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Calculando as frequências de visitação}
  ...
  \begin{ideiabox}
    É o \emph{forward--backward} dos modelos ocultos de Markov, com a exponencial da
    recompensa no lugar da emissão. E $\log Z_s$ é um \emph{máximo suave}: uma equação
    de Bellman com \textsc{softmax} no lugar do $\max$.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Teste sua compreensão}
  ...
  \pause
  \begin{respbox}
    \textbf{Não, e não.} A clonagem só precisa dos pares $(s,a)$ --- é aprendizado
    supervisionado puro, sem qualquer interação. O DAgger precisa \emph{executar} a
    política no mundo (ou em um simulador) e de um especialista para rotular, mas
    nunca precisa da distribuição $\tran{s'}{s,a}$ explicitamente.

    \smallskip
    O MaxEnt IRL, ao contrário, precisa do modelo --- ou da capacidade de simular ---
    porque tem de calcular \emph{o que a política atual faria}, e não apenas comparar
    com o que o especialista fez. É o preço de recuperar uma recompensa em vez de
    copiar um comportamento.
  \end{respbox}
\end{frame}
...
\begin{frame}{Balanço do RL inverso de máxima entropia}
  ...
  \begin{column}{0.49\textwidth}
    \begin{alertabox}
      \begin{itemize}\setlength{\itemsep}{0.1ex}
        ...
        \item Continua refém da escolha de $\feat$; versões modernas aprendem os
          atributos com redes (\emph{guided cost learning}).
      \end{itemize}
    \end{alertabox}
  \end{column}
  \end{columns}
  ...
  \begin{ideiabox}
    ...
    depois planejar, treina junto um discriminador (especialista \emph{vs.}\ aprendiz)
    e a política --- o discriminador faz o papel da recompensa.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{De recompensas aprendidas a políticas}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Métodos de RL inverso entregam uma \emph{recompensa}. Em geral o que se
      quer é uma \emph{política} cujo desempenho iguale ou supere a do especialista.
    \item Caminho direto: pegue $\hat\rec$ e rode RL comum com ela --- é o que a
      iteração de margem máxima faz internamente.
    \item Caminho mais curto: aprender a política diretamente (GAIL, e variantes
      adversariais).
  \end{itemize}
  ...
  \begin{ideiabox}
    Esse cenário --- aprender recompensa de comparações --- é o dos \emph{dueling
    bandits}, e volta em breve no curso. As duas metades de hoje eram, o tempo todo,
    as duas metades do mesmo sistema.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Mapa dos métodos de imitação}
  ...
  \begin{ideiabox}
    Uma leitura da tabela: quanto mais barata a supervisão exigida, mais frágil a
    garantia. A clonagem pede o mínimo e entrega o mínimo; o RL inverso pede um
    modelo do mundo e entrega algo transferível.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Leituras e próxima aula}
  ...
  \begin{column}{0.48\textwidth}
    ...
    \end{ideiabox}
    \begin{alertabox}
      Uma pergunta em aberto para levar: a clonagem de comportamento, por
      construção, tem o especialista como teto. O RL inverso, não. O que exatamente
      permite ultrapassar quem nos ensinou?
    \end{alertabox}
  \end{column}
  \end{columns}
\end{frame}
