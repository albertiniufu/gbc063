...
\begin{frame}{De onde viemos}{Aula 11 --- bandits, arrependimento e a visão bayesiana}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \begin{ideiabox}
        Tudo isso valia para \emph{uma única decisão} repetida. Hoje há \alert{estados}:
        uma ação ruim não custa só a recompensa daquele passo --- ela decide
        \emph{onde} você estará nos próximos mil.
      \end{ideiabox}
      \begin{alertabox}
        É essa dependência temporal que torna a exploração em MDPs
        \emph{qualitativamente} mais difícil, e não apenas mais cara.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Plano de hoje}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      \begin{enumerate}
        \item \textbf{Um novo critério: PAC.} Arrependimento mede área sob a curva de
              erro; PAC \emph{conta} decisões ruins.
        \item \textbf{Otimismo em MDPs: MBIE-EB} e sua garantia PAC.
        \item \textbf{Lema da simulação:} converte erro de \emph{modelo} em erro de
              \emph{valor}.
        \item \textbf{PSRL:} amostrar um MDP inteiro por episódio.
        \item \textbf{Generalização:} o que sobra quando não dá para contar visitas.
      \end{enumerate}
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{teobox}{A pergunta de fundo}
        Existe algoritmo que, em \emph{qualquer} MDP tabular, cometa apenas um número
        \alert{polinomial} de decisões ruins? Sim --- e o que decide isso não é a taxa
        de aprendizado nem a arquitetura.
      \end{teobox}
      \begin{ideiabox}
        No \textit{RiverSwim} adiante, $\epsilon$-guloso colhe $85$ e otimismo colhe
...
\begin{frame}{O que o arrependimento não distingue}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \end{itemize}
      \smallskip
      Em ensaio clínico, tutor inteligente ou controle de processo, a pergunta não é
      ``quanto perdi no total'', mas \alert{``em quantos momentos entreguei uma decisão
      ruim?''}
    \end{column}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{teobox}{Não são equivalentes}
        ...
        rarearem); e um algoritmo PAC pode ter arrependimento linear se as $N$ decisões
        ruins forem caríssimas.
      \end{teobox}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{ideiabox}
        Na prática, ambos são atendidos pela \emph{mesma} família de algoritmos, porque
        ambos exigem a mesma coisa: reduzir incerteza de forma \alert{dirigida}, não
        por sorteio.
      \end{ideiabox}
    \end{column}
  \end{columns}
...
\begin{frame}{Por que exploração em MDPs é outro problema}{O \textit{RiverSwim}: um contraexemplo de seis estados}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{alertabox}
        A recompensa grande está a cinco passos improváveis, e o caminho é \emph{contra}
        a dinâmica. A política ótima ($\gamma=0{,}95$) nada à direita em todo estado.
      \end{alertabox}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{ideiabox}
        Nenhuma quantidade de \emph{ruído local} produz uma \emph{sequência} coerente de
        cinco escolhas certas. Explorar bem exige um \alert{plano} de exploração.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{MBIE-EB}{\textit{Model-Based Interval Estimation with Exploration Bonus} --- Strehl \& Littman (2008)}
  ...
  \end{algbox}
  \vspace{-0.4em}
  {\small\color{rlCinza} Estime o modelo por contagens, some um \emph{bônus} que decai,
  \emph{planeje} no modelo otimista e siga o plano --- sem nenhum $\epsilon$-guloso: a
  política executada é determinística.}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{teobox}{1 --- concentração}
        ...
        $\abs{\Rhat - \rec} \le \sqrt{\ln(2/\delta')/(2n)}$ com probabilidade
        $1-\delta'$.
      \end{teobox}
      \begin{teobox}{2 --- limitante da união}
        Deve valer para $\abs{\gS}\abs{\gA}$ pares e $m$ níveis: troque $\delta'$ por
    ...
    \begin{column}{0.48\textwidth}
      ...
      \end{defbox}
      \begin{ideiabox}
        Estados nunca visitados começam com $\Qtil = \tfrac{1}{1-\gamma}$, o maior valor
        possível: o desconhecido é, \emph{por construção}, o mais atraente do mapa.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{MBIE-EB é PAC}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \begin{ideiabox}
        O passo (4) é o pivô: \emph{ser otimista transforma cada erro em informação}.
        Se a ação não é boa, $\Qtil$ está inflado em algum ponto do caminho --- e só
        infla onde falta experiência.
      \end{ideiabox}
      \begin{alertabox}
...
\begin{frame}{Retomada: bandits bayesianos}
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      \end{defbox}
      \begin{ideiabox}
        Atualizar a crença custa \emph{duas somas}. É essa conveniência algébrica que
...
\begin{frame}{Do bandit ao MDP bayesiano}
  \begin{columns}[T]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \end{defbox}
      \begin{ideiabox}
        Calcular essa probabilidade é intratável, mas \emph{amostrar} dela é trivial:
        sorteie um MDP, resolva-o, aja de forma ótima nele. A ação já sai distribuída
        assim, sem nunca calcular a integral.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{teobox}{Garantia}
        ...
        algoritmos otimistas, e em geral melhor na prática.
      \end{teobox}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{ideiabox}
        Repare no que PSRL \emph{não} tem: nenhum $\epsilon$, nenhum bônus, nenhuma
        constante de concentração. A exploração emerge da variância do
        \textit{a posteriori}, que encolhe sozinha.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{A granularidade da amostragem é o algoritmo}
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      \end{defbox}
      \begin{ideiabox}
        Comprometer-se com uma hipótese por um episódio é o análogo bayesiano da
        propagação do bônus: ambos dão \alert{exploração profunda}.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.49\textwidth}
      \centering
...
\begin{frame}{Otimismo e Thompson: duas leituras da mesma ideia}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{ideiabox}
        Ambos transformam ``não sei'' em ``vale a pena ir ver''. Otimismo faz isso
        \emph{deterministicamente e no pior caso}; Thompson, \emph{estocasticamente e
        na média}.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{teobox}{Amostragem por semente}
        Dimakopoulou \& Van Roy (2018): com $M$ agentes em paralelo, amostrar
        independentemente faz todos seguirem hipóteses parecidas. Uma \emph{semente}
        fixa por agente gera diversidade coordenada.
      \end{teobox}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{alertabox}
        ...
        Se cada estado é visitado no máximo uma vez, $n(s,a) \in \{0,1\}$: o bônus
        deixa de informar.
      \end{alertabox}
    ...
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        A generalização \emph{muda o problema de exploração}: se estados parecidos têm
        valores parecidos, visitar um informa sobre os outros. A exploração deixa de
        escalar com $\abs{\gS}$ e passa a escalar com a \emph{complexidade da classe
        de funções}.
      \end{ideiabox}
...
\begin{frame}{Bandits contextuais}
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      ...
      \end{defbox}
      \begin{ideiabox}
        Meio-termo exato entre bandit e MDP: tem \emph{estado} (exige generalização) mas
        não tem \emph{consequência} (não exige exploração profunda). É onde a teoria é
        completa --- e onde vive quase toda a aplicação industrial.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.49\textwidth}
...
\begin{frame}{Sim --- por empilhamento em blocos}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \]
      \begin{alertabox}
        O preço é a dimensão: $d \to Kd$. E esse é o ponto --- o modelo disjunto
        \alert{não compartilha informação entre braços}, e a dimensão maior é a
        expressão formal disso. Um $\feat(s,a)$ bem projetado aprende sobre um braço
        observando outro.
      \end{alertabox}
    \end{column}
    \begin{column}{0.46\textwidth}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{teobox}{A leitura teórica}
        UCB: $\tilde{O}(\sqrt{KT})$; LinUCB: $\tilde{O}(d\sqrt{T})$. O número de braços
        sai do limitante, substituído pela dimensão.
      \end{teobox}
    \end{column}
    \begin{column}{0.51\textwidth}
      ...
      \\[0.3em]
      \begin{alertabox}
        O ganho existe \emph{se o modelo linear estiver correto}. Sob má especificação,
...
\begin{frame}{Incerteza sobre um vetor}{De intervalos de confiança a elipsoides}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{ideiabox}
        O termo $\sqrt{\feat^{\!\top}A_t^{-1}\feat}$ é a largura do elipsoide \emph{na
        direção} de $\feat$: o análogo de $1/\sqrt{n(s,a)}$, sensível à geometria.
      \end{ideiabox}
      \begin{quizbox}{}
...
\begin{frame}{Incerteza epistêmica e incerteza aleatória}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \end{respbox}
      \begin{alertabox}
        Explorar para reduzir ruído aleatório é desperdício. Um bônus proporcional à
        \emph{variância total} exploraria para sempre o braço mais ruidoso, ainda que
        sua média já fosse conhecida.
      \end{alertabox}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{ideiabox}
        Esta distinção atravessa a aula inteira:
        \begin{itemize}
  ...
  \begin{columns}[T]
    \begin{column}{0.51\textwidth}
      ...
      \begin{algbox}{Com bônus de exploração}
        ...
        \]
      \end{algbox}
      {\small\color{rlCinza} Idêntico ao MBIE-EB: o bônus entra no \emph{alvo} de
      Bellman e é propagado para trás. Muda só \emph{como} calculá-lo.}
    \end{column}
    \begin{column}{0.47\textwidth}
...
\begin{frame}{Quando isso importa muito: \textit{Montezuma's Revenge}}
  \begin{columns}[T]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \end{alertabox}
      \begin{ideiabox}
        A diferença entre um agente que funciona e um que não funciona está em
        \emph{como ele decide o que ainda não sabe}.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Aprender \emph{a explorar}}{Exploração entre tarefas e meta-RL}
  \begin{columns}[T]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \end{teobox}
      \begin{ideiabox}
        Fecha-se o círculo: começamos exigindo garantias no pior caso e terminamos
        aprendendo o \textit{a priori} que o torna irrelevante.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Onde está a teoria hoje}
  \begin{columns}[T]
    ...
    \begin{column}{0.49\textwidth}
      ...
      \end{teobox}
      \begin{alertabox}
        A pergunta em aberto não é ``existe algoritmo eficiente?'', e sim \emph{quais
        propriedades do domínio} tornam a exploração tratável. Fora delas, há limitantes
        inferiores exponenciais.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
