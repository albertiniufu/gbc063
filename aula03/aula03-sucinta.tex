...
%%  mesmas definições e figuras do aula03.tex)
...
\begin{frame}{Roteiro}
  \begin{itemize}
    \item \textbf{Aula passada:} MRPs e MDPs; avaliação e controle de
      políticas \emph{com} o modelo verdadeiro ($P$ e $\rec$ conhecidas).
    \item \textbf{Hoje:} avaliar $\pol$ \alert{sem} conhecer $P$ nem
      $\rec$ — apenas interagindo com o ambiente.
    \item \textbf{Próxima aula:} \emph{controle} sem modelo — boa política
      a partir da experiência.
  \end{itemize}

  \begin{ideiabox}
    Até hoje o agente ``lia o manual do mundo''; agora aprende
    \emph{vivendo}: executa $\pol$, observa e estima o quanto ela vale.
  \end{ideiabox}
...
\begin{frame}{Avaliação por experiência direta}
  \textbf{Problema.} Estimar o retorno esperado de $\pol$ só com dados
  coletados do ambiente.
  \begin{itemize}\setlength{\itemsep}{0.2ex}
    \item Quase toda aplicação real (robótica, saúde, recomendação):
      \alert{não temos} o modelo verdadeiro.
    \item Hoje os dados vêm da própria $\pol$ (\emph{on-policy});
      \emph{off-policy} fica para adiante.
  \end{itemize}
  ...
  \begin{respbox}
    ...
    memória — a régua da aula inteira.
  \end{respbox}
\end{frame}
...
\begin{frame}{O cardápio de hoje}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Diferença Temporal (TD):} atualiza a cada passo,
      com a própria estimativa como ``palpite'' do futuro.
    \item \textbf{Equivalência de certeza:} estima modelo por máxima
      verossimilhança e planeja nele como se verdadeiro.
    \item \textbf{Avaliação em lote} (\emph{batch}): dados fixos e finitos —
      e um exemplo em que MC e TD discordam.
  \end{enumerate}

  \begin{ideiabox}
    Os três primeiros: cantos de um mesmo mapa — profundidade (quanto do
    futuro) $\times$ largura (quantos futuros); voltamos a ele no fim.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Aquecimento: iteração de valor $\times$ iteração de política}
  ...
  \pause
  \begin{respbox}
    \textbf{Verdadeiro.} Ambas convergem para $\Vstar$ e devolvem política
    com valor ótimo — os \emph{caminhos} até lá é que diferem.
  \end{respbox}
\end{frame}
...
\begin{frame}{Aquecimento: quantas iterações a iteração de valor pode levar?}
  ...
  \pause
  \begin{respbox}
    \textbf{Sim.} MDP com um único estado e uma única ação,
    $r(s,a) = 1$, $\gamma = 0{,}9$, $V_0(s) = 0$: $\abs{\gA}^{\abs{\gS}} = 1$,
    mas $\Vstar(s) = \tfrac{1}{1-\gamma} = 10$; após a primeira iteração
    $V_1(s) = 1$ — convergência \emph{assintótica}. (A iteração de
    \emph{política} para em no máximo $\abs{\gA}^{\abs{\gS}}$ passos.)
  \end{respbox}
\end{frame}
...
\begin{frame}{Programação dinâmica para avaliação (com modelo)}
  Com $P$ e $\rec$ conhecidas, Bellman dá um algoritmo iterativo:
  \[
  ...
  \begin{itemize}
    \item $V^{\pol}_k(s)$: valor de $s$ com horizonte $k$; antes da
      convergência, \emph{estimativa} de $\Vpi(s)$.
    \item Termo direito: substitui a esperança do futuro
...
\begin{frame}{O que a PD usa — e o que não teremos hoje}
  ...
  \medskip
  \begin{itemize}
    \item PD: média \textbf{sobre todos} os $s'$, ponderada por
      $\tran{s'}{s,\pol(s)}$ — precisa do modelo.
    \item Hoje só temos \alert{amostras}: um $s'$ por vez, sorteado pelo
      próprio ambiente.
  \end{itemize}

  \begin{alertabox}
    Sem modelo não dá para somar sobre $s'$ — a aula inteira contorna isso.
  \end{alertabox}
\end{frame}
...
\begin{frame}{A ideia mais simples possível}
  ...
  \begin{ideiabox}
    Valor é uma esperança — esperanças se estimam com médias:
    rode a política, anote o retorno por estado, tire a média:
    \alert{valor $=$ retorno médio}.
  \end{ideiabox}

  \begin{itemize}
    \item Não requer modelo de dinâmica nem de recompensa.
    \item Nem sequer assume que o estado é Markoviano!
    \item Requer episódios \textbf{finitos}: $G_t$ só se calcula no fim.
...
\begin{frame}{MC de primeira visita}
  ...
  Cada estado: \textbf{no máximo uma amostra por episódio} — amostras
  independentes entre episódios.
\end{frame}
...
\begin{frame}{MC de toda visita}
  ...
  \begin{itemize}
    \item Política revisita $s$ no episódio: cada visita gera uma amostra
      de retorno.
    \item Amostras \emph{correlacionadas} (compartilham o fim do episódio) —
      consequências estatísticas adiante.
  \end{itemize}
\end{frame}
...
\begin{frame}{Exemplo resolvido: solução}
  ...
  \medskip
  Além disso: $V(s_3) = \gamma^3$, $V(s_1) = 1$; nunca visitados
  ($s_4$ a $s_7$) permanecem na inicialização.
\end{frame}

%% ------------------------------------------------------------
\begin{frame}{MC incremental: a mesma média, sem guardar somas}
  A média se atualiza amostra a amostra. Ao visitar $s$ e observar
  $G_{i,t}$, com $N(s) \leftarrow N(s)+1$:
  \[
  ...
  \begin{ideiabox}
    O molde de quase tudo no curso:
    $\;\text{estimativa} \leftarrow \text{estimativa}
      + \alpha\,(\text{alvo} - \text{estimativa})$ —
    muda-se o alvo, nasce um algoritmo.
  \end{ideiabox}
\end{frame}

%% ------------------------------------------------------------
\begin{frame}{Interlúdio estatístico: como julgar um estimador?}
  Modelo com parâmetro $\theta$ gera dados $x \sim P(x \mid \theta)$;
  estimador $\hat{\theta} = \hat{\theta}(x)$ tenta recuperar $\theta$.
...
\begin{frame}{Viés zero implica consistência?}
  ...
  \pause
  \begin{respbox}
    \textbf{Não.} Estimar a média de uma Gaussiana usando \emph{apenas a
    primeira} observação, $\hat{\theta}_n = x_1$: não-viesado
    ($\E[x_1] = \theta$), mas ignora os demais dados — a distribuição do
    erro não encolhe. Consistência exige \emph{aproveitar} os dados novos.
  \end{respbox}

  \begin{ideiabox}
    Viés é ``mirar no lugar errado''; variância é ``tremer a mão'' —
    o EQM cobra os dois.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Propriedades dos estimadores MC}
  \begin{itemize}
    ...
    \item \textbf{Toda visita:} \alert{viesado} (amostras correlacionadas
      no episódio), porém \textbf{consistente} — com frequência EQM
      \emph{menor} na prática.
    \item \textbf{Incremental:} propriedades dependem da taxa $\alpha$.
  \end{itemize}

  \begin{teobox}{Condições de convergência (Robbins–Monro)}
    Com $\alpha$ variando por atualização
...
\begin{frame}{Limitações do Monte Carlo}
  \begin{alertabox}
    $G_t$ acumula a aleatoriedade de \emph{todas} as ações, transições e
    recompensas até o fim do episódio: \textbf{variância alta} em geral.
  \end{alertabox}

  \begin{itemize}
    \item Reduzir a variância pode exigir \emph{muitos} dados — inviável
      quando coletar é caro, lento ou arriscado (medicina, robôs, educação).
...
\begin{frame}{Resumo: Monte Carlo}
  \begin{itemize}
    \item Estima a esperança $\Vpi(s) = \E_{\pol}[G_t \mid s_t = s]$
      por média empírica de retornos.
    \item Simples; não requer modelo; \textbf{não assume Markov}.
    \item Converge para o valor verdadeiro sob hipóteses brandas.
    \item Às vezes preferível à PD \emph{mesmo com o modelo} — $\gS$
      gigantesco e só interessam alguns estados, ou estado não Markoviano.
  \end{itemize}
\end{frame}
...
\begin{frame}{A ideia central do RL}
  ...
  \emph{a} ideia mais central e original de toda a RL.

  \medskip
  \begin{itemize}
    \item Combinação de MC (\textbf{amostra}) com PD (\textbf{bootstrapping}).
    \item Livre de modelo.
    \item Episódico \emph{e} horizonte infinito.
    \item Atualiza $V$ \alert{imediatamente} após cada tupla $(s, a, r, s')$.
  \end{itemize}

  \begin{ideiabox}
    MC espera o episódio acabar para saber ``quanto valeu''; TD usa a
    recompensa de um passo e o \emph{próprio palpite} sobre o próximo estado.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Algoritmo TD(0)}
  ...
  \begin{itemize}
    \item Cada tupla é usada uma vez, na hora — sem armazenar episódios
      nem esperar que terminem.
    \item Por isso serve para tarefas contínuas, sem estados terminais.
    \item ``(0)'' indica olhar 1 passo à frente; família TD($\lambda$)
      interpola TD(0) e Monte Carlo.
  \end{itemize}
\end{frame}
...
\begin{frame}{Exemplo resolvido: TD no Mars Rover}
  ...
  $\pol(s) = a_1$ para todo $s$; $s_1, s_7$ terminam o episódio.
...
\begin{frame}{MC e TD divergem — no mesmo episódio!}
  Para a mesma trajetória (tomando $\gamma = 1$):
  \begin{center}
  ...
  \begin{itemize}
    \item MC propaga o desfecho $+1$ \textbf{para trás}, a todos os
      estados visitados no episódio.
    \item TD(0) usa cada tupla \textbf{uma única vez} — quando $s_2$ foi
      atualizado, $V(s_1)$ ainda era $0$: a notícia não teve tempo de
      se propagar.
    \item Com mais episódios (ou repassando os dados), TD também propaga;
      a diferença é a \emph{velocidade} de influência de cada dado.
  \end{itemize}
\end{frame}
...
\begin{frame}{Teste sua compreensão: o papel de $\alpha$}
  ...
  \pause
  \begin{respbox}
    \textbf{2, 3 e 4.} Com $\alpha = 0$ nada é atualizado (1 falsa);
...
\begin{frame}{Resumo: Diferença Temporal}
  \begin{itemize}
    \item Amostra do ambiente (como MC) \textbf{e} bootstrapping (como PD).
    \item Livre de modelo; episódico e contínuo.
    \item Atualiza a cada tupla $(s, a, r, s')$ — aprendizado \emph{online}.
    \item Estimador \alert{viesado}: no começo, o alvo depende da
      inicialização de $V$.
    \item Variância \textbf{bem menor} que a do MC: o alvo contém uma única
      ação, recompensa e transição aleatórias.
    \item Consistente se $\alpha$ satisfaz Robbins–Monro (as mesmas do MC
      incremental).
  \end{itemize}
\end{frame}
...
\begin{frame}{Equivalência de certeza: custo e propriedades}
  \begin{itemize}
    \item \textbf{Custo alto:} replanejar a cada atualização —
      $O(\abs{\gS}^3)$ analítico, ou $O(\abs{\gS}^2 \abs{\gA})$ por
      iteração.
    \item \textbf{Eficiência estatística excelente:} cada tupla melhora o
      modelo inteiro; o planejamento espreme toda a informação.
    \item \textbf{Consistente} quando o processo é de fato Markoviano.
    \item Estende-se à avaliação \emph{off-policy}: o modelo estimado não
      depende de qual política gerou os dados.
  \end{itemize}

  \begin{ideiabox}
    O nome diz tudo: tratamos a estimativa \emph{como se} houvesse certeza —
    eficiente com poucos dados, caro em computação: o oposto exato do TD.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Exemplo resolvido: equivalência de certeza no Mars Rover}
  ...
  estados — nenhum método anterior conseguiu.
\end{frame}
...
\begin{frame}{O cenário em lote (\emph{batch})}
  Dados em fluxo até aqui. Agora: conjunto \textbf{fixo e finito} de $K$
  episódios coletados sob $\pol$:
  \vspace{-0.5em}
  \[
  ...
  \begin{itemize}\setlength{\itemsep}{0.2ex}
    \item Regime típico quando coletar dados novos é caro ou arriscado
      (prontuários médicos, logs de produção).
...
\begin{frame}{Por que discordam — e o que cada um ``acredita''}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      ...
      partir de $A$, e ele valeu $0$: $V(A) = 0$ — ajusta-se aos
      \alert{retornos que aconteceram}.
    \end{column}
    \begin{column}{0.50\textwidth}
      \small\textbf{TD(0).} Nos dados, $A$ sempre levou a $B$ com $r=0$:
      $V(A) = 0 + \gamma V(B) = 0{,}75$ — o TD \alert{acredita na
  ...
  \begin{ideiabox}
    Mundo Markoviano: a resposta do TD aproveita melhor os dados; se não,
    a do MC é a mais honesta.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O modelo que o TD constrói sem querer}
  ...
  \begin{itemize}\setlength{\itemsep}{0.2ex}
    ...
    \item De $B$, seis vezes a recompensa foi $1$ e duas vezes $0$
  ...
  \begin{ideiabox}
    ...
    a mesma resposta que a equivalência de certeza daria — o preço e o
...
\begin{frame}{Viés e variância dos alvos}
  ...
  \begin{itemize}
    ...
    \item \textbf{TD(0):} viesado, variância menor; converge na
      representação tabular, mas \alert{pode não convergir com aproximação
      de função} (tema de aulas futuras).
  \end{itemize}

  \begin{ideiabox}
    Dilema viés–variância clássico em roupa de RL: o TD troca um pouco de
    viés por aprender mais rápido e com menos ruído.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Tabela comparativa}
  ...
  \smallskip
  \begin{alertabox}
    Sem vencedor absoluto: a escolha depende do recurso escasso — dados,
    computação ou memória — e de o estado ser Markoviano.
  \end{alertabox}
\end{frame}
...
\begin{frame}{O que levamos desta aula}
  \begin{itemize}
    \item Avaliação de política \textbf{sem modelo}: esperanças por
      \emph{amostras} (MC), \emph{amostra + bootstrapping} (TD) ou
      \emph{modelo estimado} (equivalência de certeza).
    \item O molde universal
      $\;\text{estimativa} \leftarrow \text{estimativa}
        + \alpha(\text{alvo} - \text{estimativa})\;$
      reaparece na aula que vem — trocando $V$ por $Q$ e avaliação por
      \textbf{controle}.
...
\begin{frame}{Exercício: complete o quadro do Mars Rover}
  ...
  \begin{ideiabox}
    Contraste: o TD usou cada tupla uma vez e propagou pouco; o MC
    propagou o desfecho para todo o episódio; a equivalência de certeza
    extraiu o máximo — ao preço de inverter uma matriz.
  \end{ideiabox}
\end{frame}
