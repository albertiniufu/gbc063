...
\begin{frame}{Plano de hoje}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.55ex}
    \item \textbf{Avaliação de política em lote} — o que MC e TD calculam
      quando os dados acabam.
    \item \textbf{Melhoria de política generalizada} — $Q$ e não $V$;
      explorar sem perder a melhoria monotônica.
    ...
    \item \textbf{Aproximação de função valor} — abandonar a tabela para
      \emph{generalizar}.
    \item \textbf{Deep Q-Learning} — o que faltava para funcionar em Atari.
  \end{enumerate}

  \begin{ideiabox}
    Fio condutor: mesma linha de código,
    $\;$\emph{estimativa} $\leftarrow$ \emph{estimativa} $+\;\alpha\,($\emph{alvo} $-$ \emph{estimativa}$)$,
    com quatro escolhas de \emph{alvo}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Onde estamos}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Até aqui, tudo \textbf{tabular}: uma entrada por estado
      (ou por par estado-ação).
    \item Segunda metade quebra a hipótese: $\abs{\gS}$ grande demais para
      memória — quanto mais para visitar.
  \end{itemize}
\end{frame}
...
\begin{frame}{Recapitulação: os três estimadores da Aula 3}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    \item \textbf{Monte Carlo} — trajetória completa $\tau$; retorno observado $G_t$:
      \[ \Vpi(s) \leftarrow \Vpi(s) + \alpha\bigl(G_t(s) - \Vpi(s)\bigr) \]
    \item \textbf{TD(0)} — tupla $(s,a,r,s')$; \emph{bootstrap} na própria estimativa:
      \[ \Vpi(s) \leftarrow \Vpi(s) + \alpha\bigl(\underbrace{r + \gamma \Vpi(s')}_{\text{alvo TD}} - \Vpi(s)\bigr) \]
    \item \textbf{Equivalência de certeza} — estima $\Phat$ e $\rhat$ por
      máxima verossimilhança; PD sobre o MDP estimado.
  \end{itemize}

  \begin{ideiabox}
    Os três já apareceram como \emph{procedimentos}. Hoje: para \emph{qual
    objeto} cada um converge com dados finitos e reciclados?
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Do fluxo para o lote}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Antes: cada tupla $(s,a,r,s')$ usada \textbf{uma vez} e descartada.
    \item Em lote: $K$ episódios armazenados, reprocessáveis à vontade.
  \end{itemize}
...
\begin{frame}{O exemplo A--B}
  ...
  \begin{columns}[T]
    \begin{column}{0.44\textwidth}
      ...
      \medskip
      {\small Cada episódio termina após a última recompensa. $A$ aparece
      \textbf{uma única vez} na base.}
    \end{column}
...
\begin{frame}{O exemplo A--B: as duas respostas}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \smallskip
      \textbf{Monte Carlo.} Único retorno de $A$: $G = 0 + 0 = 0$. MC
      minimiza o erro quadrático sobre retornos \emph{observados} → $0$.

      \smallskip
      \textbf{TD(0).} Alvo $r + \gamma V(B) = 0{,}75$: encadeia a
      informação de $B$, mesmo com $A$ uma vez só.
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{center}
      ...
      \vspace{-0.7em}
      \begin{ideiabox}
        MDP de \textbf{máxima verossimilhança} implícito nos dados; TD(0) em
        lote converge para o $\Vpi$ dele — por isso $V(A) = 0{,}75$.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Para onde MC e TD convergem no cenário em lote}
  ...
  \begin{teobox}{TD(0) em lote}
    ...
    dados --- \textbf{idêntico à equivalência de certeza}:
    {\small
    \[
...
\begin{frame}{Qual das duas respostas é ``a certa''?}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{ideiabox}
        Ambiente \textbf{markoviano} → TD certo: o que se aprende sobre $B$
        vale para \emph{qualquer} caminho até $B$. TD usa a estrutura; MC
        a ignora.
      \end{ideiabox}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{alertabox}
        Ambiente \textbf{não-markoviano} (observação parcial) → TD propaga
        hipótese falsa. MC, só retornos completos, mais robusto — ao custo
        de variância.
      \end{alertabox}
    \end{column}
  \end{columns}
  ...
  \smallskip
  {\small MC correto \emph{sem} supor Markov; TD supõe e, quando vale,
  converge com muito menos dados.}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        ...
        \item MC: $O(L)$, mas só \emph{depois} do fim do episódio.
        \item Eq.\ de certeza: sistema linear a cada atualização — muito
          mais caro.
      \end{itemize}
      ...
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        \item Markov (TD, eq.\ de certeza) extrai mais de cada amostra; MC,
          quando ela não vale.
      \end{itemize}
    \end{column}
...
\begin{frame}{Fecho da avaliação de política}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item \textbf{Monte Carlo} — dados \emph{on-policy} ou \emph{off-policy}
      (amostragem por importância).
    \item \textbf{Diferença temporal} — TD(0) e variantes.
    \item \textbf{PD com equivalência de certeza} — construir o modelo e
      resolvê-lo.
  \end{itemize}
  ...
  \begin{ideiabox}
    Avaliação é metade do problema; agora \emph{escolher} a política — e o
    obstáculo novo: dados sobre ações que a política atual \textbf{não}
    toma.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Iteração de política sem modelo}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \vspace{-0.3em}
      \begin{alertabox}
        Três buracos: (1) $\pol$ determinística → $Q(s,a)$ com $a \neq
        \pol(s)$ nunca observado; (2) melhoria usa $\Qhat$ ruidoso; (3)
        quanto avaliar antes de melhorar?
      \end{alertabox}
    \end{column}
...
\begin{frame}{Por que $Q$ e não $V$}
  ...
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Com modelo, melhoria gulosa a partir de $V$:
      \[ \pol'(s) = \argmax_{a \in \gA}\; \Bigl[\rec(s,a) + \gamma \sum_{s'} \tran{s'}{s,a} V(s')\Bigr] \]
    \item Sem modelo, $\rec$ e $P$ desconhecidos — expressão inútil. Com $Q$:
      \[ \pol'(s) = \argmax_{a \in \gA}\; Q(s,a) \]
      melhoria imediata, sem conhecimento da dinâmica.
  \end{itemize}

  \begin{ideiabox}
    $Q$ = $V$ com a \emph{informação de um passo do modelo embutida}. Por
    isso praticamente todo controle sem modelo aprende $Q$.
  \end{ideiabox}
...
\begin{frame}{O problema da exploração}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Explorar}
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        \item Aprender exige experimentar.
    ...
    \begin{column}{0.5\textwidth}
      \textbf{Explotar}
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        \item Passo explorando = passo sem colher recompensa conhecida.
  ...
  \vspace{-0.2em}
  {\small \textbf{Gulosa pura} sobre $\Qhat$ pode travar para sempre:
  primeira amostra de uma ação boa ruim por azar → nunca mais escolhida,
  nem corrigida.}
...
\begin{frame}{Políticas $\epsilon$-gulosas}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Toda ação com probabilidade $\ge \epsilon/\abs{\gA} > 0$ —
      exploração garantida.
    \item $\epsilon = 1$: passeio aleatório. \quad $\epsilon = 0$: guloso puro.
    \item Alternativa: \emph{softmax}, $\pol(a\mid s) \propto e^{Q(s,a)/\tau}$,
      que explora ações \emph{quase} boas.
  \end{itemize}
\end{frame}
...
\begin{frame}{A melhoria monotônica sobrevive à exploração}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Aula 2: iteração de política com $P$ e $\rec$ conhecidos melhora
      monotonicamente — prova supunha política \textbf{determinística}.
    \item $\epsilon$-gulosa é estocástica de propósito. A garantia se perde?
  \end{itemize}
  ...
  \vspace{-0.4em}
  \begin{ideiabox}
    Explorar \emph{para sempre} sem nunca piorar; custo: o ótimo entre as
    $\epsilon$-gulosas carrega o $\epsilon$ — resíduo que a condição GLIE
    elimina.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Prova da melhoria monotônica $\epsilon$-gulosa}
  ...
  \vspace{-0.5em}
  \begin{itemize}\setlength{\itemsep}{0.2ex}
    \item[] {\footnotesize Desigualdade: máximo $\ge$ \emph{qualquer}
      combinação convexa; pesos $\ge 0$ pois $\pol_i$ é $\epsilon$-gulosa
      ($\pol_i(a\mid s) \ge \epsilon/\abs{\gA}$).}
    \item[] {\footnotesize De $Q^{\pol_i}(s,\pol_{i+1}(s)) \ge V^{\pol_i}(s)$
      segue $V^{\pol_{i+1}} \ge V^{\pol_i}$ (teorema de melhoria, Aula 2).
      \hfill $\blacksquare$}
  \end{itemize}
\end{frame}
...
\begin{frame}{Exemplo trabalhado: MC-controle no \emph{Mars Rover}}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    ...
    \item Início: $Q(s,a) = 0$, $\pol(s) = a_1$, $\epsilon = 0{,}5$.
    \item Trajetória amostrada da $\epsilon$-gulosa:
...
\begin{frame}{Exemplo trabalhado: solução}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \smallskip
      {\footnotesize Primeiras visitas antes da recompensa $1$ em $s_1$: com
      $\gamma=1$, cada par recebe retorno $1$.}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}{Guloso no limite com exploração infinita (GLIE)}
  ...
  {\small As duas condições puxam em direções opostas. \textbf{Estratégia
  GLIE mais simples}: $\epsilon$-guloso com $\epsilon_i = 1/i$ —
  $\sum_i 1/i$ diverge, $\epsilon_i \to 0$.}
...
\begin{frame}{Controle com métodos de diferença temporal}
  ...
  {\small Vantagem: sem esperar o episódio — tarefas \textbf{contínuas}.
  Pergunta que separa os dois algoritmos: \textbf{qual $Q$ do próximo
  estado entra no alvo?}}

  \vspace{-0.4em}
  \begin{ideiabox}
    \textbf{SARSA} usa a ação \emph{de fato tomada}. \textbf{Q-learning}
    usa a \emph{melhor} ação, tomada ou não. Um símbolo de diferença,
...
\begin{frame}{\emph{On-policy} e \emph{off-policy}}
  ...
  \medskip
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Off-policy permite aprender de \textbf{dados de arquivo}:
      prontuários, logs, demonstrações.
    \item Permite reutilizar experiência antiga — base do \emph{replay} do
      DQN; também fonte da instabilidade com aproximação de função.
  \end{itemize}
\end{frame}
...
\begin{frame}{Convergência do Q-learning tabular}
  ...
  \begin{ideiabox}
    Primeira soma: erros iniciais \emph{corrigíveis}; segunda: ruído
    \emph{amortecido}. Ex.: $\alpha_t = 1/t$.
  \end{ideiabox}

  {\small Prova: aproximação estocástica, contração do backup de Bellman,
  valores limitados. \alert{Nenhuma das três sobrevive intacta à
...
\begin{frame}{SARSA {\normalfont vs.} Q-learning no mesmo passo}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      {\footnotesize Usa $a_2$: ação de fato tomada em $s_7$,
    ...
    \begin{column}{0.5\textwidth}
      ...
      {\footnotesize Usa $\max\{10,5\} = 10$, ignorando a ação de fato
      escolhida.}
    \end{column}
  \end{columns}

  \begin{alertabox}
    Inicialização de $Q$ importa? \textbf{Assintoticamente não} — mas
    \textbf{muito} no início. Inicializar otimisticamente é, aliás,
    estratégia de exploração.
  \end{alertabox}

  \begin{ideiabox}
    Um símbolo separa ``seguir esta política, com exploração e tudo'' de
...
\begin{frame}{O que essa diferença provoca: o \emph{cliff walking}}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \end{tikzpicture}}
      \end{center}
      {\footnotesize $r = -1$ por passo; penhasco: $-100$ e volta a I.
      Se $\epsilon \to 0$, ambos convergem à política ótima — mas
      \emph{durante} o aprendizado, a cautela do SARSA costuma ser
      preferível.}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.3ex}
        \item {\color{rlLaranja}\textbf{Q-learning}} aprende o caminho
          \emph{ótimo}, rente ao penhasco; agindo $\epsilon$-guloso, cai às
          vezes — \textbf{recompensa média pior}.
        \item {\color{rlAzul}\textbf{SARSA}} aprende o caminho \emph{seguro},
          rota de cima: o alvo já contabiliza os passos exploratórios.
      \end{itemize}
...
\begin{frame}{Teste sua compreensão: SARSA e Q-learning}
  ...
  \pause
  \begin{respbox}
    \textbf{As duas são verdadeiras.} (1) Ambos incrementais: a política
    gulosa muda assim que $Q$ muda. (2) $\epsilon = 0$ →
    $a_{t+1} = \argmax_{a'} Q(s_{t+1},a')$; alvos coincidem. Perde-se a
    garantia de convergência, que exige GLIE.
  \end{respbox}
\end{frame}
...
\begin{frame}{Um viés escondido no $\max$}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Alvo usa $\max_{a'} \Qhat(s',a')$ para estimar
      $\max_{a'} Q(s',a')$. Máximo de estimativas ruidosas: \textbf{enviesado
      para cima}:
      \[ \E\Bigl[\max_{a'} \Qhat(s',a')\Bigr] \;\ge\; \max_{a'} \E\bigl[\Qhat(s',a')\bigr] \]
      (Jensen: $\max$ é convexo).
    \item Ação com sorte nas amostras → o $\max$ a seleciona; erro
      propagado pelo \emph{bootstrapping}.
  \end{itemize}


  \begin{ideiabox}
    Correção: \textbf{desacoplar seleção e avaliação} — um conjunto de
    estimativas escolhe o $\argmax$, outro o avalia. Ideia do \emph{Double
    Q-learning} — e, depois, do \emph{Double DQN}.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Por que a tabela não basta}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.52\textwidth}
      \begin{alertabox}
        Não é só memória: mesmo cabendo, seria preciso \emph{visitar} cada
        entrada ao menos uma vez para estimá-la.
      \end{alertabox}

      \begin{ideiabox}
        Palavra-chave: \textbf{generalização} — aprender sobre um estado e,
        com isso, saber de estados parecidos nunca visitados.
      \end{ideiabox}
    \end{column}
  \end{columns}

  \smallskip
  {\small Evitar armazenar por estado o modelo $(P,\rec)$, o valor $V$, o
  valor de ação $Q$ e a própria política $\pol$ — reduz \textbf{memória},
  \textbf{computação} e, sobretudo, a \textbf{experiência} necessária.}
\end{frame}
...
\begin{frame}{O problema, primeiro com um oráculo}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Suponha um oráculo que devolve o valor verdadeiro $Q^{\pol}(s,a)$
      para qualquer par consultado.
    \item Então: regressão comum sobre pares
      $\bigl((s,a),\, Q^{\pol}(s,a)\bigr)$.
    \item Objetivo: $\wv$ na família $\Qhat(s,a;\wv)$ que melhor aproxima
      $Q^{\pol}$.
  \end{itemize}
  ...
  → passo por amostra $\Delta \wv = -\tfrac{1}{2}\alpha \nabla_{\wv} J(\wv)$:}
  \vspace{-0.6em}
  \[ \Delta \wv = \alpha\bigl(Q^{\pol}(s,a) - \Qhat(s,a;\wv)\bigr)\nabla_{\wv}\Qhat(s,a;\wv). \]
  \vspace{-1.3em}

  {\small \textbf{SGD}: uma (ou poucas) amostras no lugar da esperança. Em
...
\begin{frame}{Aproximadores lineares e o caso tabular}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Atributos por par: $\xv(s,a) = [x_1(s,a), \dots, x_d(s,a)]^{\top}$, com
...
\begin{frame}{Sem oráculo: Monte Carlo com aproximação de função}
  ...
  {\small $G_t$: amostra \textbf{não-enviesada} (ruidosa) de
  $Q^{\pol}(s_t,a_t)$ → substitui o oráculo: regressão sobre os pares
  $\langle (s_t,a_t), G_t\rangle$. Linear: mínimo global da perda
  quadrática.}
...
\begin{frame}{TD(0) com aproximação de função}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Tabular: $\Vpi(s) \leftarrow \Vpi(s) + \alpha(r + \gamma \Vpi(s') - \Vpi(s))$
      — alvo $r + \gamma\Vpi(s')$ já \emph{enviesado}.
    \item Com aproximação, alvo $r + \gamma\Vhat^{\pol}(s';\wv)$:
      enviesado \textbf{e} aproximado.
  \end{itemize}

  \begin{alertabox}
    \begin{enumerate}\setlength{\itemsep}{0.2ex}
      \item \textbf{Amostragem} --- transição no lugar da esperança sobre $P$.
      \item \textbf{Bootstrapping} --- própria estimativa no lugar do valor
        verdadeiro.
      \item \textbf{Aproximação de função} --- projeção numa família
        paramétrica que talvez não contenha $\Vpi$.
    \end{enumerate}
  \end{alertabox}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    ...
    \item \textbf{Detalhe importante:} alvo tratado como \emph{constante}
      ao derivar → não é o gradiente verdadeiro de $J$: \textbf{semigradiente}.
  \end{itemize}
\end{frame}
...
\begin{frame}{TD(0) com aproximação: o algoritmo}
  ...
  \begin{ideiabox}
    Derivar o alvo → \emph{gradiente residual}: estável, lento, exige duas
    amostras independentes do próximo estado. Semigradiente: mais rápido —
    ao custo das garantias.
  \end{ideiabox}
...
\begin{frame}{Controle com aproximação de função valor}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item Valores de ação: $\Qhat^{\pol}(s,a;\wv) \approx Q^{\pol}$.
    \item Intercalamos, como sempre:
      \begin{itemize}\setlength{\itemsep}{0.15ex}
        \item avaliação aproximada: ajustar $\wv$;
        \item melhoria $\epsilon$-gulosa sobre $\Qhat$.
      \end{itemize}
    \item Sem $Q^{\pol}(s_t,a_t)$: substituímos por um \textbf{alvo},
      exatamente como no tabular:
  \end{itemize}
  ...
  \smallskip
  {\small Uma linha de código, três algoritmos: a Seção 3 troca ``entrada
  da tabela'' por ``saída da rede''.}
\end{frame}
...
\begin{frame}{A ``tríade mortal''}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.58\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.25ex}
        \item Cada atualização: backup de Bellman aproximado, \emph{projetado}
          sobre a família de funções.
        \item Backup: \textbf{contração}; projeção: pode ser \textbf{expansão}.
          A composição pode não contrair — oscila ou diverge.
        \item Dois a dois: seguros. Os três juntos: não.
      \end{itemize}
      \vspace{-0.5em}

      \begin{alertabox}
        Contraexemplo: \textbf{Baird} (Sutton \& Barto, 2018) — Q-learning
        linear diverge, pesos crescem sem limite.
      \end{alertabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Mapa das garantias de convergência}
  ...
  \vspace{-0.6em}
  \begin{ideiabox}
    Q-learning com rede neural: sem garantia alguma. O DQN não resolve
    teoricamente — ele \emph{engenheira} o problema para funcionar na
    prática.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Q-learning com redes neurais: o que dá errado}
  ...
  {\small Tabular: converge para $\Qstar$. Com aproximação: minimiza EQM
  por SGD usando alvo estimado — pode divergir.}
  ...
  \begin{alertabox}
    ...
    \begin{enumerate}\setlength{\itemsep}{0.25ex}
      ...
      \item \textbf{Alvos não-estacionários.} O alvo usa os mesmos pesos
        que atualizamos: perseguimos um alvo que se move.
    \end{enumerate}
  \end{alertabox}
...
\begin{frame}{Repetição de experiência}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item \textbf{Buffer de repetição} $\mathcal{D}$: tuplas $(s,a,r,s')$
      já vividas (tipicamente $10^6$ transições, janela deslizante).
    \item Repita:
      \begin{enumerate}\setlength{\itemsep}{0.15ex}
        \item amostre $(s,a,r,s') \sim \mathcal{D}$ (ou um \emph{minibatch});
        \item alvo $r + \gamma \max_{a'} \Qhat(s',a';\wv)$;
        \item atualize
  ...
  \begin{ideiabox}
    Só possível porque Q-learning é \textbf{off-policy}: tuplas de
    políticas antigas não invalidam o alvo (que já usa o $\max$, não a ação
    escolhida). SARSA não pode sem correção. Resta o segundo problema: o
    alvo muda quando os pesos mudam.
  \end{ideiabox}
...
\begin{frame}{Alvos fixos}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item \textbf{Dois} conjuntos de pesos:
      \begin{itemize}\setlength{\itemsep}{0.15ex}
        \item $\wv$ --- pesos atualizados;
        \item $\wtar$ --- pesos da \emph{rede-alvo}, cópia congelada.
      \end{itemize}
    \item Alvo calculado com $\wtar$:
  ...
  \begin{ideiabox}
    Entre sincronizações: regressão supervisionada comum, com rótulos
    fixos — a manobra que devolve estabilidade a quem, de outro modo,
...
\begin{frame}{DQN: o algoritmo completo}
  ...
  {\footnotesize Escolhas de projeto embutidas: arquitetura da rede, taxa
  de aprendizado, frequência $C$ de sincronização, tamanho do buffer e
  como preenchê-lo antes de começar a aprender.}
\end{frame}
...
\begin{frame}{Teste sua compreensão: alvos fixos}
  ...
  \pause
  \begin{respbox}
    \textbf{Apenas a segunda.} Memória dobra: dois conjuntos de pesos. O
    tempo \emph{não} dobra: em ambos os casos, uma passagem para frente
    (alvo) + uma para frente/para trás (atualização). O extra é só a cópia
    $\wtar \leftarrow \wv$ a cada $C$ passos.
  \end{respbox}
\end{frame}
    ...
    \begin{column}{0.45\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.25ex}
        \item Estado $s$: pilha dos \textbf{4 últimos quadros} de pixels
          brutos — velocidade e direção, que um quadro único não revela.
        \item Saída: um $\Qhat(s,a)$ para cada uma das 18 posições de
...
\begin{frame}{Quais ingredientes do DQN realmente importavam?}
  ...
  \begin{alertabox}
    Rede profunda, \textbf{sozinha}, chegou a \emph{piorar} (Enduro: 62
    $\to$ 29; Seaquest: 656 $\to$ 275). A diferença foram o \emph{replay} e
    o alvo fixo.
  \end{alertabox}
...
\begin{frame}{Por que o \emph{replay} ajuda tanto}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.5ex}
    \item \textbf{Descorrelaciona} o \emph{minibatch} — aproxima a hipótese
      i.i.d.\ que o SGD supõe.
    \item \textbf{Reaproveita} cada transição muitas vezes — eficiência de
      dados, o recurso caro quando cada amostra exige interação.
    \item \textbf{Estabiliza a distribuição de treino}: sem buffer, mudança
      de política altera os dados na hora, e a rede ``esquece'' regiões que
      deixou de visitar (esquecimento catastrófico).
  \end{enumerate}

  \begin{ideiabox}
    Terceiro efeito, o mais subestimado: o buffer impede otimizar apenas
    para o presente. Replay sozinho já salta Breakout de $3 \to 241$.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Aperfeiçoamentos imediatos do DQN}
  ...
  \begin{itemize}\setlength{\itemsep}{0.5ex}
    ...
    \item \textbf{Repetição priorizada} (Schaul et al., ICLR 2016) ---
      amostrar com probabilidade proporcional ao erro TD: aprender mais
      onde ainda se erra.
    \item \textbf{Dueling DQN} (Wang et al., ICML 2016) --- decompor
      $\Qhat(s,a) = \Vhat(s) + \widehat{A}(s,a)$ em valor de estado e
      vantagem: aprende $\Vhat(s)$ mesmo com ação irrelevante.
  \end{itemize}
...
\begin{frame}{Próxima aula}
  ...
  \begin{itemize}\setlength{\itemsep}{0.45ex}
    ...
    \item Limites: ações contínuas tornam o $\argmax$ caro; políticas
      estocásticas ótimas não representáveis dessa forma.
